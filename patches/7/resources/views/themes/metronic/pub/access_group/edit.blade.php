                ...
                <div class="card">
                    <div class="card-header min-h-auto py-5 border-bottom">
                        <div class="card-title flex-column align-items-start">
                            <h4 class="fw-bold mb-0">Редактирование доступа</h4>
                        </div>
                    </div>
                    <form action="{{ route('access_group.update', $group) }}" method="POST"
                        ...
                        <div class="card-body">
                            ...
                            <div class="mb-3 row">
                                <label for="tb-code" class="col-sm-4 col-form-label fw-semibold text-lg-end">Префикс <span class="text-danger">*</span></label>
                                <div class="col-sm-4">
                                    <input name="prefix" type="text" class="form-control " id="tb-code"
                                           placeholder="" required="" value="{{ old('prefix') ?? $group->prefix }}">
                            ...
                            <div class="mb-3 row">
                                <label for="tb-icon"
                                       class="col-sm-4 col-form-label fw-semibold text-lg-end">Иконка (css class)</label>
                                <div class="col-sm-1">
                                    <input name="icon" type="text" class="form-control" id="tb-icon"
                                           placeholder="" value="{{ old('icon') ?? $group->icon }}"
                                           value="{{ old('icon') }}">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class=" modal-header modal-colored-header bg-danger text-white">
                    <h4 class="modal-title" id="danger-header-modalLabel">Удаление группы доступов</h4>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Не удалять"></button>
                </div>
                <div class="modal-body">
                    <h5 class="mt-0">Внимание!</h5>
                    <p>
                        Удаление группы доступов необратимо!
                        <br/>
                        Вы можете удалить эту группу, потому что в ней нет доступов
                    </p>
                </div>
                <div class="modal-footer">
                    ...
                    <form method="POST" action="{{ route('access_group.delete', $group   ) }}">
                        @method('DELETE')
                        @csrf
                        <button
                            type="submit"
