...
class PaymentCalendarController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // по умолчанию показываем спецификации в работе; оплаченные платежи
        // видны при любом статусе (см. PaymentCalendarService::rows)
        $spec_status = $request->has('spec_status')
    ...
    protected static function chips(array $params): array
    {
        ...
        foreach ($params['spec_status'] as $status) {
            $ret[] = [
                'key' => 'spec_status',
                'value' => $status,
                'label' => 'спец.: ' . (PaymentCalendarService::specStatuses()[$status] ?? $status),
            ];
        }
