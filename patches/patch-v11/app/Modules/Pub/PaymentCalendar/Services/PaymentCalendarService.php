...
class PaymentCalendarService
{
    ...
    public static function rows(array $params = []): Collection
    {
        ...
        // статус спецификации: по умолчанию только «в процессе»,
        // но уже оплаченные платежи видны при любом статусе — деньги пришли
        $spec_status = static::listParam($params['spec_status'] ?? null, false);
        if (!empty($spec_status)) {
            $rows = $rows->filter(
                fn($row) => in_array((string) $row->spec_status, $spec_status, true) || $row->state === 'paid'
            );
        }
