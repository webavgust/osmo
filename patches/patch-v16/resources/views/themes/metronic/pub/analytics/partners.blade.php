...
@section('content')
    <div class="container-fluid">
        ...
        {{-- Легенда: что такое балл и буквы --}}
        <div class="card mb-4">
            <div class="card-body py-4">
                <div class="row g-4">
        ...
        <div class="card">
                            ...
                            <tr>
                                ...
                                <td class="text-center text-nowrap">
                                    <a href="javascript:void(0)" class="num-link fw-bold fs-4 text-dark"
                                       onclick="javascript:box({href:'{{ $box('stats') }}'})">{{ $row['proposals'] }}</a>
                                    <div class="fs-8 text-muted">
                                ...
                                {{-- Срок от последнего выставленного КП до спецификации --}}
                                <td class="text-center text-nowrap">
                                    @if($row['days_to_spec'] !== null)
                                        <span class="fw-bold">
                                            {{ \App\Modules\Pub\Analytics\Services\PartnerScoringService::humanPeriod($row['days_to_spec']) }}
                                        </span>
                                        <div class="fs-8 text-muted">{{ $row['days_to_spec'] }} дн. в среднем</div>
                                    @else
                                        <span class="text-muted">—</span>
            ...
            <div class="card-footer py-3 fs-8 text-muted">
                ...
                последнего выставленного КП до прикрепления его к спецификации. Суммы приведены
                к рублям по текущему курсу.
            </div>
        </div>
    </div>
@endsection
