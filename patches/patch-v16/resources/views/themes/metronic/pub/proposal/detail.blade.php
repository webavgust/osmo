...
@section('content')
    ...
    <div class="container-fluid" id="proposal" currency="RUB">
        <div class="row">
            ...
            <div class="col-11">
                <div class="d-flex align-items-top justify-content-between mb-2 gap-3">
                    <div>
                        <h2 class="m-0">
                            {{ $proposal->name }}
                        ...
                        <div class="fs-4 mt-1">{{ $proposal->partner->name }}</div>
                    </div>

                    {{-- Сводная, статус и привязка к сделке — один ряд кнопок --}}
                                    ...
                                    >
                                        <div class="card card-body p-0">
                                            <div class="
                                                ...
                                                {{-- Валюта, номер, дата и компания-заказчик --}}
                                                <div class="d-flex align-items-start justify-content-end ms-auto gap-3">
                                                    @if($proposal->isForeignCurrency)
                                                    ...
                                                    <div class="text-end">
                                                        ...
                                                        <div class="fs-2">от {{ $proposal->sended_at->format("d.m.Y") }}</div>

                                                        @if(!empty($proposal->company))
                                                            <a href="{{ route('company.detail', $proposal->company) }}" class="fs-2 text-nowrap">
                                                                <x-ui.icon.regular icon="fa-building" class="me-1"/>{{ $proposal->company->name }}
                                                            </a>
                                                        @endif

                                                        {{-- рамочные договоры, в спецификации которых лежит это КП --}}
                                                        @foreach($frame_specs as $frame_spec)
                                                            @php $frame_type = \App\Modules\Pub\Contract\Models\ContractType::tryFrom((string) $frame_spec->contract?->type)?->data(); @endphp
                                                            <div class="fs-2 text-nowrap" title="Спецификация: {{ $frame_spec->name }}">
                                                                <x-ui.icon.regular icon="fa-file-contract" class="me-1"/>
                                                                <span class="text-{{ $frame_type['color'] ?? 'dark' }}">{{ $frame_type['label'] ?? 'Договор' }}</span>
                                                                <code class="ms-1">{{ $frame_spec->contract->number ?? 'б/н' }}</code>
                                                            </div>
                                                        @endforeach
                                                    </div>
                                                </div>
