...
class PartnerScoringService
{
    ...
    public static function partner($partner, Collection $proposals, Collection $specs, Collection $payments, Collection $links, ?int $year = null): array
    {
        ...
        // отбор по году: КП — по дате отправки, спецификации — по дате
        // прикрепления КП, иначе по дате рамочного договора
        if ($year) {
    ...
    /**
     * Спецификации партнёров с датой прикрепления КП
     *
     * @return Collection
     */
    public static function specifications(): Collection
    {
        ...
        return $rows = collect(DB::table('contract_specifications as s')
            ...
            ->select([
                's.id', 's.name', 's.amount', 's.is_signed', 's.status', 's.currency_slug', 's.company_id',
                ...
                DB::raw('MIN(l.attached_at) as attached_at'),
            ...
            ->map(function ($row) {
                ...
                $row->attached_at = $row->attached_at ? Carbon::parse($row->attached_at) : null;
                return $row;
            });
    }
    ...
    /**
     * Привязки КП к спецификациям с датой отправки КП
     *
     * @return Collection
     */
    public static function links(): Collection
    {
        ...
        return $rows = collect(DB::table('contract_specification_proposals as l')
            ...
            ->join('contracts as c', 'c.id', '=', 's.contract_id')
            ->select([
                'l.id', 'l.proposal_group', 'l.attached_at',
    ...
    /**
     * Год спецификации: по прикреплению КП, иначе по дате договора
     *
     * @param mixed $spec
     * @return int|null
     */
    ...
    {
        return ($spec->attached_at ?? $spec->contract_date)?->year;
    }

    /**
     * Дата, к которой относим привязку
     *
     * @param mixed $link
     * @return Carbon|null
     */
    ...
    {
        return $link->attached_at ?? $link->contract_date;
    }

    /**
     * Дней от последнего выставленного КП до спецификации
     *
     * @param mixed $link
     * @return int|null
     */
