...
class CrmDealRegistryService
{
    ...
    public static function params($input = null): array
    {
        ...
        $has_proposal = (string) ($input['has_proposal'] ?? static::DEFAULTS['has_proposal']);
        if (!array_key_exists($has_proposal, static::HAS_PROPOSAL)) {
            $has_proposal = static::DEFAULTS['has_proposal'];
        }
     ...
     * @return Collection
     */
    public static function rows(array $params = [], ?Partner $partner = null): Collection
    {
        ...
        return $rows->map(function ($row) use ($proposals) {
