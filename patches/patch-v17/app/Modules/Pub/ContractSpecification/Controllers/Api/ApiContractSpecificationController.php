...
//            'amount' => 'nullable|int',
     ...
     *
     * Ограничения проверяются здесь же, а не только в попапе: компания КП
     * должна совпадать с компанией спецификации, а блок — с типом рамочного
     * договора.
     *
    ...
    public function set_proposal(ContractSpecification $spec, Request $request)
    {
        ...
        if (!SpecProposalService::hasBlock($proposal, $block['relation'] ?? null)) {
            return [
                'result' => 'error',
                'message' => 'В этом КП нет блока «' . ($block['label'] ?? '—') . '»',
            ];
        }
