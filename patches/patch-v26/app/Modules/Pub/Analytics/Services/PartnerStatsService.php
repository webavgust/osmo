...
class PartnerStatsService
{
    ...
    public static function partnerYears(int $partner_id): array
    {
        ...
        // сделки и проекты тоже дают балл, значит и год (patch v26)
        PartnerScoringService::deals()
            ->where('partner_id', $partner_id)
            ->each(fn($row) => $years->push($row->year_ref));

        PartnerScoringService::projects()
            ->where('partner_id', $partner_id)
            ->each(fn($row) => $years->push($row->year_ref));
    ...
    /**
     * Все КП партнёра — расшифровка колонки «КП»
     *
     * @param int $partner_id
     * @param int|null $year
     * @return Collection
     */
    public static function proposals(int $partner_id, ?int $year = null): Collection
    {
    ...
    /**
     * Сделки Битрикса — расшифровка колонки «Сделки» (patch v26)
     *
     * @param int $partner_id
     * @param int|null $year
     * @return Collection
     */
    public static function deals(int $partner_id, ?int $year = null): Collection
    {
        return PartnerScoringService::deals()
            ->where('partner_id', $partner_id)
            ->filter(fn($row) => !$year || $row->year_ref === $year)
            ->sortByDesc(fn($row) => $row->date_create)
            ->values();
    }

    /**
     * Проекты — расшифровка колонки «Проекты» (patch v26)
     *
     * @param int $partner_id
     * @param int|null $year
     * @return Collection
     */
    public static function projects(int $partner_id, ?int $year = null): Collection
    {
        return PartnerScoringService::projects()
            ->where('partner_id', $partner_id)
            ->filter(fn($row) => !$year || $row->year_ref === $year)
            ->sortByDesc(fn($row) => $row->date_start)
            ->values();
    }

    /**
     * Партнёр сопоставлен с компанией Битрикс24: без сопоставления сделок
     * и проектов у него не будет, сколько бы их ни было в Битриксе
     *
     * @param int $partner_id
     * @return bool
     */
    public static function crmLinked(int $partner_id): bool
    {
        return !empty(PartnerScoringService::companyMap()[$partner_id] ?? []);
    }
