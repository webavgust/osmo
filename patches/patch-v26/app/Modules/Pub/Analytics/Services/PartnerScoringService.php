 ...
 *
 * Патч v26: состав балла пересобран по ТЗ 2026-09. «Количество КП за год»
 * заменено на сделки Битрикса, добавлены проекты, а «размер ожидаемых
 * платежей» из балла убран — он остаётся справочной цифрой в таблице и в
 * попапе. Итоговые веса:
 *
 *   сумма подписанных спецификаций   вес 35
 *   количество проектов              вес 25
 *   конверсия решённых КП            вес 25
 *   кол-во сделок битрикс            вес 10
 *   доля просроченных платежей       вес 5 (чем меньше, тем выше балл)
 *
 * Сделки живут в отдельной базе (соединение bitrix), а связь с партнёром —
 * в нашей (`partner_crm_companies`), поэтому кросс-базовый JOIN невозможен:
 * связь собирается в PHP двумя запросами. Партнёр без сопоставления с
 * Битрикс24 получает 0 сделок — это не ошибка, а отсутствие сопоставления.
 */
class PartnerScoringService
{
    ...
    public static function metrics(?int $year = null): Collection
    {
        ...
        $deals = static::deals()->groupBy('partner_id');
        $projects = static::projects()->groupBy('partner_id');

        // сделки и проекты дают почти половину балла, поэтому партнёр, у
        // которого в портале нет ни КП, ни спецификаций, всё равно попадает
        // в рейтинг — иначе его работа была бы не видна (patch v26)
        $ids = collect()
            ->concat($proposals->keys())
            ->concat($specs->keys())
            ->concat($deals->keys())
            ->concat($projects->keys())
            ->unique()
            ->filter()
            ->values();
        ...
        return $ids
            ->map(fn($id) => static::partner(
                ...
                deals: $deals->get($id, collect()),
                projects: $projects->get($id, collect()),
                year: $year
            ))
     ...
     * @param Collection $deals Сделки Битрикса партнёра (patch v26)
     * @param Collection $projects Проекты партнёра (patch v26)
     * @param int|null $year
     * @return array
     */
    public static function partner($partner, Collection $proposals, Collection $specs, Collection $payments, Collection $links, Collection $deals, Collection $projects, ?int $year = null): array
    {
        ...
        if ($year) {
            ...
            // сделки — по дате создания, проекты — по дате начала (patch v26)
            $deals = $deals->filter(fn($row) => $row->year_ref === $year);
            $projects = $projects->filter(fn($row) => $row->year_ref === $year);
        }
        ...
        return [
            ...
            // сделки Битрикса и проекты (patch v26)
            'deals' => $deals->count(),
            'deals_sum' => $deals->sum(fn($row) => (float) $row->amount * static::rate($row->currency_slug)),
            'crm_linked' => !empty(static::companyMap()[(int) $partner->id] ?? []),

            'projects' => $projects->count(),
            'projects_archived' => $projects->filter(fn($row) => !empty($row->archived_at))->count(),
            'projects_pilot' => $projects->filter(fn($row) => !empty($row->is_pilot))->count(),
    ...
    public static function components(Collection $rows): Collection
    {
        ...
        return $rows->map(function ($row) use ($best_specs, $best_projects, $best_deals) {
            ...
            $deals = $best_deals > 0 ? $row['deals'] / $best_deals * 100 : 0;
    ...
    /**
     * Сопоставление партнёров с компаниями Битрикс24 (patch v26)
     *
     * @return array id партнёра → [id компаний Битрикса]
     */
    public static function companyMap(): array
    {
        static $map = null;
        ...
        DB::table('partner_crm_companies')
            ->whereNotNull('partner_id')
            ->whereNotNull('crm_company_id')
            ->get(['partner_id', 'crm_company_id'])
            ->each(function ($row) use (&$map) {
                $map[(int) $row->partner_id][] = (int) $row->crm_company_id;
            });
    ...
    /**
     * Сделки Битрикса, отнесённые к партнёрам портала (patch v26).
     *
     * Сделки лежат в базе `avgbitrix`, сопоставление партнёра с компанией —
     * в нашей, поэтому JOIN между ними невозможен: карта «компания → партнёр»
     * собирается в PHP, а сделки выбираются одним запросом по списку компаний.
     * Стадия и год не ограничиваются — считаются все сделки партнёра,
     * а отбор по году делает partner().
     *
     * @return Collection
     */
    public static function deals(): Collection
    {
        ...
        // компания Битрикса → партнёр портала
        $owners = [];
        foreach (static::companyMap() as $partner_id => $companies) {
            foreach ($companies as $company_id) $owners[$company_id] = $partner_id;
        }

        if (empty($owners)) return $rows = collect();

        return $rows = collect(DB::connection('bitrix')->table('crm_deal')
            ->whereIn('company_id', array_keys($owners))
            ->select([
                'id', 'title', 'company_id', 'company_name', 'stage_name',
                'opportunity', 'currency_id', 'date_create',
            ])
            ->orderByDesc('date_create')
            ->get())
            ->map(function ($row) use ($owners) {
                $row->partner_id = $owners[(int) $row->company_id] ?? null;
                $row->date_create = $row->date_create ? Carbon::parse($row->date_create) : null;
                $row->year_ref = $row->date_create?->year;

                // приводим поля к вокабуляру остальных выборок скоринга
                $row->amount = (float) $row->opportunity;
                $row->currency_slug = $row->currency_id;

                return $row;
            })
            ->filter(fn($row) => !empty($row->partner_id))
            ->values();
    }

    /**
     * Проекты партнёров (patch v26).
     *
     * Год проекта — по дате начала; архивные считаются наравне с
     * действующими (решение владельца).
     *
     * @return Collection
     */
    public static function projects(): Collection
    {
        ...
        return $rows = collect(DB::table('deal_projects as p')
            ->leftJoin('companies as cm', 'cm.id', '=', 'p.company_id')
            ->whereNotNull('p.partner_id')
            ->select([
                'p.id', 'p.partner_id', 'p.company_id', 'p.date_start', 'p.deadline',
                'p.is_pilot', 'p.archived_at', 'p.comment',
                'cm.name as company_name',
                DB::raw('(SELECT COUNT(*) FROM deal_project_deals d WHERE d.deal_project_id = p.id) as deals_count'),
            ])
            ->get())
            ->map(function ($row) {
                $row->date_start = $row->date_start ? Carbon::parse($row->date_start) : null;
                $row->deadline = $row->deadline ? Carbon::parse($row->deadline) : null;
                $row->archived_at = $row->archived_at ? Carbon::parse($row->archived_at) : null;
                $row->year_ref = $row->date_start?->year;
