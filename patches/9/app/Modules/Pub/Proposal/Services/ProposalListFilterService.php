            ...
            $builder->where(function ($builder) use ($filter) {
                foreach ($this->filter as $field => $value) {
                    switch ($field) {
                        ...
                        // --- наличие привязанных сделок Битрикса ------------
                        // считаем по таблице привязок: у КП их может быть несколько
                        case "crm_deal":
                            if($value === 'linked') {
                                $builder->whereExists(function ($query) {
                                    $query->from('proposal_crm_deals')
                                        ->whereColumn('proposal_crm_deals.proposal_group', 'proposals.group');
                                });
                            } elseif($value === 'empty') {
                        ...
                        case "crm_deal_id":
                            ...
                            if($ids->isNotEmpty())
                                $builder->whereExists(function ($query) use ($ids) {
                                    $query->from('proposal_crm_deals')
                                        ->whereColumn('proposal_crm_deals.proposal_group', 'proposals.group')
                                        ->whereIn('proposal_crm_deals.crm_deal_id', $ids->all());
                                });
                        break;
                    }
                }
