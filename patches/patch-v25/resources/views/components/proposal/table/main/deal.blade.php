@php
     ...
     *
     * data-deal-cell — метка для перерисовки ячейки без перезагрузки списка
     * (patch v25): попап привязки находит по ней свою строку.
     */
    $links = \App\Modules\Pub\Proposal\Services\ProposalDealService::links($row);
    $main = $links->first();
    $url = route('proposal.box_deal', [$row, $row->iteration]);
@endphp
<div class="cell" data-deal-cell="{{ $row->group }}">
