...
/**
 * Статус КП и привязка к сделкам Битрикса.
 *
...
class ApiProposalStatusController
{
    ...
    /**
     * Привязать сделку. main=1 — сделать её главной
     * (главную дублируем в proposals.crm_deal_id для старого кода).
     *
     * @param Request $request
     * @param Proposal $proposal
     * @param int $iteration
    ...
    public function dealAttach(Request $request, Proposal $proposal, int $iteration)
    {
        ...
        return [
            'result' => 'success',
            'count' => ProposalDealService::dealIds($proposal->group)->count(),
        ];
    }

    /**
     * Отвязать сделку. Без deal_id снимает все привязки.
     *
     * @param Request $request
     * @param Proposal $proposal
     * @param int $iteration
     * @return array
     */
    public function dealDetach(Request $request, Proposal $proposal, int $iteration)
    {
        $proposal = ProposalRepository::getOnce($proposal->group, $iteration);
        if (empty($proposal)) abort(404);

        ProposalDealService::detach($proposal, (int) $request->input('deal_id') ?: null);
    ...
    /**
     * Ячейка «Сделка» списка КП (patch v25).
     *
     * Попап привязки закрывается и перерисовывает одну ячейку вместо
     * перезагрузки страницы: список КП пагинируется на сервере, и
     * location.reload() сбрасывал бы страницу и сортировку.
     *
     * @param Proposal $proposal Любая итерация КП
     * @return array
     */
    public function dealCell(Proposal $proposal)
    {
        $row = ProposalRepository::getLast($proposal) ?? $proposal;

        return [
            'result' => 'success',
            'html' => view('components.proposal.table.main.deal', ['row' => $row])->render(),
        ];
    }
}
