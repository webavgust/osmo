...
class ProposalPriceHistoryService
{
    ...
    public static function iterations(string $group): Collection
    {
        ...
        return $proposals->map(function ($proposal) use (&$prev) {
            ...
            $row = [
                ...
                // валюта берётся из варианта: у редакций она может отличаться,
                // поэтому суммы никогда не показываем без её кода
    ...
    {
        return strtoupper((string) ($variant->currency_slug ?? $proposal?->currency_slug ?? 'RUB'));
    }
    ...
    {
        $from = (float) $diff->sum(fn($item) => (float) ($item['from']['total'] ?? 0));
        $to = (float) $diff->sum(fn($item) => (float) ($item['to']['total'] ?? 0));
