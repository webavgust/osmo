...
{
    /**
     * Блок КП → тип рамочного договора, которым этот блок закрывается.
     *
     * Работает в одну сторону: когда спецификацию подбирают из шапки блока
     * на странице КП, предлагаются договоры подходящего типа. Обратно (при
     * прикреплении КП к спецификации) блоки не проверяются — менеджеру нужны
     * все КП компании.
     */
    public const BLOCK_TYPES = [
        'services' => ['label' => 'Работы', 'relation' => 'proposal_works'],
        'license' => ['label' => 'ПЛАТФОРМА (ПО)', 'relation' => 'proposal_software'],
     ...
     * @return array|null ['label' => ..., 'relation' => ...]
     */
    public static function blockType(?string $block): ?array
    {
        return static::BLOCK_TYPES[(string) $block] ?? null;
    }

    /**
     * КП, которые можно прикрепить к этой спецификации.
     *
     * Все КП компании, кроме уже прикреплённых к этой спецификации: отбор
     * по блокам убран: состав КП не всегда совпадает с типом договора, а список
     * из-за этого оказывался пустым. Поиск в попапе — как у сделок Битрикса.
     *
    ...
    /**
     * Спецификации, к которым можно прикрепить это КП.
     *
     * Спецификации компании КП; если указан блок — только по договорам
     * соответствующего типа (из шапки «РАБОТЫ» предлагаются договоры по услугам).
     *
     * @param Proposal $proposal
     * @param string|null $block services|license|platform
     * @return Collection
     */
    public static function availableSpecs(Proposal $proposal, ?string $block = null): Collection
    {
        if (empty($proposal->company_id)) return collect();

        $busy = ContractSpecificationProposal::where('proposal_group', $proposal->group)
        ...
        return ContractSpecification::query()
            ...
            ->whereNotIn('id', $busy)
            ->when(!empty($block), fn($query) => $query->whereHas(
                'contract',
                fn($query) => $query->where('type', $block)
            ))
            ->with(['contract', 'currency', 'payments'])
            ->get()
            ->sortByDesc(fn($spec) => $spec->date_create ?? $spec->contract?->date)
            ->values();
    }
     ...
     * На отбор при прикреплении не влияет — оставлено для подсказок и отчётов.
     *
     * @param Proposal $proposal
     ...
     *
     * Считается по дате отправки последней редакции (это и есть «последнее
     * выставленное КП») до даты самой спецификации; если у спецификации
     * своей даты нет — по дате рамочного договора. Дата прикрепления КП
     * в расчёте не участвует: она говорит о том, когда данные завели в портал,
     * а не о том, когда состоялась сделка.
     *
    ...
    public static function daysToSpec(ContractSpecificationProposal $link, ?Proposal $proposal = null): ?int
    {
        ...
        $to = $spec?->date_create ?? $spec?->contract?->date;
