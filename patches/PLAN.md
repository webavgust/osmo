...
| 11 | v33 | Связка КП «главное / второстепенное» (см. «Итерация 13») | готов 23.09.2026, коммит `b353974`, **выкачен на прод 23.09.2026 ~12:00** (миграция `proposal_links`, пара ВДНХ связана сценарием `link_vdnh.php`, копии `/root/backup/avgmom-2026-09-23_11-57/11-59.sql.gz`); README `patches/patch-v33/README.md` |
| 12 | v34 | Рабочий стол: задачи в блокноте, календарь как в Windows 11, сайдбар события, стартовые столы (5 пресетов, `desktop:presets`), аудит всех 80 виджетов | готов 23.09.2026, коммит `ede25df`, **выкачен на прод 23.09.2026 ~15:22** (миграции `user_notes.done_at` и SRD→SAR, `desktop:presets` — пресеты #3–#7, воркер очереди supervisor `osmo_queue`, КП AM516 закрыто, копии `/root/backup/avgmom-2026-09-23_15-11/15-21.sql.gz`); README `patches/patch-v34/README.md` (открытые вопросы — там же) |
| 13 | v35 | Аудит и правка дизайна темы Metronic: 43 страницы (включая макет PDF из КП) в 5 режимах — читаемость текста («чернила»), тёмная тема, широкие таблицы, макет PDF, напоминания/календарь/уведомления, телефон | выкачен на прод 25.09.2026 (прод 79c7614); README `patches/patch-v35/README.md` (решения владельца — там же) |
| 14 | v36 | AJAX API без сессии: `ajax.api` отвечает `{error: 'auth'}` вместо 500 (сломанный вход по токену убран, решение владельца) | выкачен на прод 25.09.2026 (прод 79c7614); README `patches/patch-v36/README.md` |
| 15 | v38 | Удалены мёртвые страницы «Рабочий график» и «Привязка аналитика к объектам» (7 маршрутов, 18 файлов, точечно — контроллеры, сервис, модель User); v37 — за Битриксом | выкачен на прод 25.09.2026 (прод 79c7614); README `patches/patch-v38/README.md` |
| 16 | v39 | Список пользователей: `/api/user/list_table` 500 (`User::groups()`) → 200 — из `UserRepository::getTable()` убраны фильтр и счётчики групп/подразделений; «null» вместо отчества в двух шаблонах `pub/user` | коммит `547381a`, **выкачен на прод 25.09.2026** (БД не менялась); README `patches/patch-v39/README.md` |
| 17 | v40 | Воронка продаж без сделок Wei Wei [195] и Александра Суханова [191] — все таблицы, попапы, фильтр, «Сделки с ошибками» и выгрузки таблиц воронки; реестр и рабочий стол не меняются | коммит `9bb742a`, **выкачен на прод 25.09.2026** (БД не менялась; прод `5948ca8` вместе с правкой цены сценария в КП `a5eb3aa`); README `patches/patch-v40/README.md` |
...
- **15.09.2026, по просьбе владельца удалены все проекты на проде**: 21 проект (все от
  `deal-project:seed`, руками созданных не было), 21 связь со сделками, 6 со спецификациями и журнал
  проектов (21 событие, 107 строк). Перед удалением — копия `/root/backup/avgmom-2026-09-15_00-31.sql.gz`.
  Проекты на проде теперь не создаются сами: `deal-project:seed` — только по решению владельца.
- **API Алексея (15.09.2026):** с прода по-прежнему `302` на проверку cookie AI Studio. Промпт для его
  LLM — `patches/patch-v21/PROMPT_ALEXEY_API.md`; после нового адреса — `OSMOVIEW_CP_URL` в `.env`
  прода и `php artisan external-proposal:sync`, код не меняется.

## Следующий этап — WIKI-документация по сайту (с 15.09.2026)

Владелец: новый раздел в корпоративной WIKI про портал; читатели — люди и корпоративная
LLM (запросы вида «где найти отчёт по партнёрам»). Полный гайд по работе с сайтом со
скриншотами ключевых блоков и схемами; без чувствительных данных, без админской части.
Решения владельца: формат — Markdown в репозитории (`docs/wiki/`), скриншоты снимать
с локальной базы как есть (внутренняя wiki), Playwright ставить.

- Структура и правила страниц — `docs/wiki/_GUIDE.md` (обязательные заголовки «Где найти»,
  «Для чего», «Кому доступно», «Как …», «Частые вопросы» — по ним LLM находит ответ).
- Страницы 01–14 пишут агенты по 2–3 файла (по вёрстке и контроллерам, без браузера),
  каждый кладёт спецификацию кадров в `docs/wiki/img/shots/<страница>.json`; глоссарий, README,
  FAQ (15) и съёмка — сам.
- Съёмка: `patches/wiki-tools/shot.js` (Playwright + **WebKit**: Edge/Chromium зарезаны политикой
  `RemoteDebuggingAllowed=0`, Firefox из Playwright падает по side-by-side `mozglue`). Перед кадром
  скрипт открывает `/ui-theme/metronic` (тема живёт в cookie, иначе старая тема и 500 на страницах,
  которых в ней нет), прячет debugbar. Рабочая копия инструмента — scratchpad `shots/`.
- Стол Анны локально почти пуст (владелец тестирует) — для кадров рабочего стола нужен отдельный
  стол; стол владельца не сохранять.
- **Сделано 15.09.2026:** `docs/wiki/` — README (оглавление, карта меню, схема данных), глоссарий,
  страницы 01–14, FAQ (15), `_GUIDE.md` (правила), `_TODO.md` (что проверить владельцу: пометки
  агентов и две сломанные страницы — `/user/{id}` без таблицы `user_sub_users` и рабочий календарь
  без класса `UserWorkCalendar`). Кадры: ~95 PNG в `docs/wiki/img/`, спецификации в
  `docs/wiki/img/shots/*.json` (+README с плейсхолдерами), инструмент `patches/wiki-tools/`
  (`shot.js`, `wiki_desk.php` — временный стол для кадров, удалён после съёмки).
  Скрипт прячет админские пункты профиля и раздел «Настройки» в меню, кадры редактирования
  стола — на ширине 1920. Не закоммичено — по решению владельца.
- Просмотр в браузере: `docs/wiki/index.html` (docsify, библиотеки с jsdelivr) + `_sidebar.md`,
  сервер — preview `osmo-wiki` (`python -m http.server 8124` в `docs/wiki`). Портал на `:8123`
  отдаёт только `public/`, поэтому ссылки вида `localhost:8123/docs/wiki/…` не открываются.
- **Ревизия 15.09.2026.** Владелец: «мы не описываем страницу для слепого… сосредоточиться надо на
  действиях». `_GUIDE.md` переписан (задачи с заголовками-глаголами, «Как это устроено», частые
  вопросы; без перечней полей, колонок и расположения), образец — `01-interface.md`. Переписаны все
  страницы 01–14 и сверен FAQ. Шесть агентов на ревизию упали (обрывы API, зависания) — всё сделано
  вручную. Кадров оставлено 28 из 99, лишние удалены вместе со спецификациями. Дефекты и открытые
  вопросы, найденные по ходу, — `docs/wiki/_TODO.md`.
- **Архив для специалиста, который разворачивает вики (15.09.2026):**
  `storage/app/wiki-export/osmo-wiki-2026-09-15.zip` (вне git) — 17 страниц, 28 кадров, `DEPLOY.md`
  с порядком страниц и тем, что учесть при переносе (ссылки, картинки, Mermaid, HTML-таблицы, доступ,
  индексация для LLM), просмотрщик `index.html`. Служебные `_GUIDE.md` и `_TODO.md` не входят.
  Сборка — `python patches/wiki-tools/export/build.py <zip>`.

## Внешние КП на проде (16.09.2026, в работе)

Владелец: «нужно выгрузить external proposals на продакшн… мне нужны только записи в БД из выгрузки
по API, чтобы на проде раздел был не пустой».

- Локально `external_proposals` — 27 записей, у всех есть детали (payload), выгружены из API
  09.09.2026 21:05; 5 из них локально тестово перенесены в КП. Выгрузка для прода без привязок
  к КП (`proposal_group`, `transferred_*` = null) — scratchpad `external_proposals.json`,
  скрипт `ep_local_export.php`.
- **На проде таблица НЕ пустая:** те же 27 записей (external_id совпадают поимённо, та же дата
  выгрузки, `proposal_group` у всех null), `external_scenario_map` — 16 строк, миграции применены.
  Переносить записи не нужно.
- Значит, «пустой раздел» на проде — другая причина. Следующий шаг: проверить на проде через ядро
  (шаблон scratchpad `prod_smoke.php`, запуск `runuser -u www-root -- php`) страницу
  `/external-proposals` и `GET /api/external-proposal/list_table` под пользователем 1 и 2
  (у API middleware `ajax.api` — нужен заголовок X-Requested-With), плюс число записей с payload.
  Спросить владельца, под каким пользователем он видел пустой раздел.
- SSH на прод иногда отвечает «timed out during banner exchange» — повтор с `-o ConnectTimeout=30`.

## Правка багов (с 16.09.2026)

- **Платёжный календарь: КП строки — из спецификации (16.09.2026).** Календарь брал КП договора
  (`contracts.proposal_id`), карточка партнёра — привязку спецификации
  (`contract_specification_proposals`): у «С.4_Лидары. перенос точки» (спецификации 101, 108,
  договоры L-PG-303.25 / S-PG-313.25) календарь показывал PG611, карточка — AK736. Решение владельца:
  «нужно брать из спецификации». Если своего КП у спецификации нет — подсказкой КП договора (последняя
  редакция) серым с пометкой «из договора» (выбор владельца; в базе ничего не меняли). Поиск по КП
  ищет так же. Правка: `PaymentCalendarService::rows()` + `attachProposals()`, вьюха
  `pub/payment_calendar/index.blade.php`. Локально из 91 платежа: 21 с КП спецификации, 39 с
  подсказкой из договора, 31 без КП. Виджеты «Финансов» на той же выборке — `render_widgets.php`
  без ошибок.
  **Ещё берут КП из договора, не тронуто:** отчёт «Сценарии по спецификациям»
  (`ReportService::getScenariosSpecsSummary`, строки спецификаций) и отчёт «Китай» (сумма КП ключа,
  `ChinaReportService`).
  **Выкачено 16.09.2026:** коммит `42fe1f2`, на проде `git pull` + `optimize:clear`; календарь и
  рабочий стол под пользователем 1 — 200 без ошибок. Правки владельца на проде (`CurrencyService.php`,
  `pub/proposal/index.blade.php`) не закоммичены ни локально, ни на проде — не трогали.
  **Данные прода расходятся с локальной базой:** привязка AK736 к спецификациям 101 и 108 есть только
  локально (создана 10.09.2026 23:32 пользователем 1); на проде у С.4 своего КП нет, поэтому там
  календарь показывает PG611 «из договора», а карточка партнёра — пусто. На проде 15 привязок
  (локально 13), последняя — 16.09.2026 14:58.
- **Форма создания КП: мелкий шрифт у «Альтернативного названия» сценария (22.09.2026).** В
  `pub/proposal/create.blade.php` поле `scenario[i][mnemonic_name]` было `fs-2`, в форме
  редактирования — `fs-7`; выровнено на `fs-7`. Не выкачено.
- **Воронка продаж: поквартальные таблицы — текущий квартал и три следующих (22.09.2026).** Таблицы
  «Страна — статус — квартал» и «Менеджер — статус — квартал» держали зашитый список
  `2026q1…2026q4`. Владелец: прошлые кварталы не интересны, «выводим текущий и 3 следующих».
  Теперь `DashboardDataService::quarterColumns()` (от `startOfQuarter()` текущей даты, формат поля
  Битрикса `2026q3`); вьюхи, окна расшифровки и выгрузка в Excel берут колонки из данных — их не
  меняли. На 22.09.2026: `2026q3, 2026q4, 2027q1, 2027q2`. Проверено: страница, окна расшифровки по
  кварталам 2027 года (200), обе выгрузки Excel. Владелец сам проверит, почему в прошлых кварталах
  остались сделки. Не выкачено.
- **Анализ скидок: несколько статусов КП в фильтре (22.09.2026).** Поле «Статус КП» — select2 с
  `multiple`, параметр `status[]`; контроллер `AnalyticsController::discounts()` принимает и список,
  и старое одиночное `?status=won`, неизвестные коды отбрасывает; `DiscountAnalysisService::rows()` —
  `whereIn`. Скрытые поля формы года уносят все выбранные статусы. Проверено: 2026 год, «в работе» 43
  + «проиграно» 11 = 54 в выборке по двум статусам; «Применить» и смена года статусы не теряют.
  Не выкачено.
- **Реестр лицензий: ключи без спецификации пропадали из отбора по партнёру (22.09.2026).** Владелец:
  у Softline в реестре 15 ключей, в отчёте `/report/license_keys` — гораздо больше. Реестр
  (`LicenseRegistryService::rows()`) брал партнёра только через спецификацию → договор, отчёт — через
  компанию ключа (`companies.partner_id`). У 60 ключей из 100 спецификации нет — в реестре они были без
  партнёра. Теперь партнёр = `COALESCE(c.partner_id, cm.partner_id)`: у привязанного ключа — партнёр
  договора, как было, у остальных — партнёр компании. Softline: 34 активных / 45 всего, как в отчёте
  (было 15 / 19); ключей без партнёра — 0. Виджет рабочего стола `license_registry` на той же выборке —
  `render_widgets.php` без ошибок. **Остался один ключ-расхождение (id 147):** компания «ЦОДД» (id 42)
  в справочнике за LURE IT, а её спецификация 101 — в договоре L-PG-303.25 МТ Интеграции; реестр
  относит ключ к МТ Интеграции, отчёт — к LURE IT. Это данные (ARCHITECTURE.md, проблема 4 — ЦОДД
  заведён у двух партнёров), решает владелец. Не выкачено.
- **Курсы валют: показывался самый старый курс, а на проде курсы не обновлялись (22.09.2026).**
  1. `CurrencyRepository::getRates(['returnFull' => true])` делал `keyBy('slug')` по выборке «от новых
     к старым» и оставлял последнюю строку — курс от 01.01.2025. Задевало «Отчёт (Китай)» (курсы из полей
     уходят и в выгрузку) и окно «Выбрать валюту» на воронке. Теперь `unique('slug')->keyBy('slug')`.
  2. `currency:update` (exchangerate-api) стоял в расписании `everyMinute()`: месячный лимит бесплатного
     ключа (1500 запросов) уходил за сутки, дальше API отвечал `quota-reached`, команда молча выходила.
     На проде с 14.05.2026 курсы есть только за 6 дней (22–23.07, 22–23.08, 15–16.09), ещё 22 дня нет
     в мае 2025. Новый ключ от 15.09 тоже исчерпан. Решение владельца: **курсы ЦБ РФ раз в день** —
     в расписании `currency:fetch-rates --from=<сегодня−7> --to=<сегодня>` в 09:00 МСК,
     `withoutOverlapping()`; недельное окно добирает дни, когда ЦБ не ответил. `currency:update` из
     расписания убран (команда осталась). История с 2025 года и так была курсами ЦБ.
  3. **SRD — не саудовский риял.** В портале «SRD (Саудовский реал)», но SRD — код суринамского доллара
     (~2,2 ₽), саудовский риял — SAR (~22,4 ₽). exchangerate-api отдавал курс суринамского доллара, то
     есть суммы в SRD занижались примерно в 10 раз. ЦБ SRD не публикует — в `FetchCurrencyRates`
     добавлено соответствие `SAR → SRD`. SRD стоит у 2 КП, в сделках Битрикса и спецификациях его нет.
     Переименовать валюту в SAR по всей базе — решение владельца.
  Локально догружено 14.05–22.09.2026 (132 дня, пропусков нет), курсы на 22.09 совпадают с ЦБ.
  Локальный PHP не находит сертификаты (`curl.cainfo` указывает в OpenServer) — запускать с
  `-d curl.cainfo="C:/Program Files/Git/mingw64/etc/ssl/certs/ca-bundle.crt"`.
  Ключ exchangerate-api зашит в `CurrencyService.php` (правка владельца, не закоммичена) — после
  перехода на ЦБ он не нужен.
- **Выкачено 22.09.2026:** коммит `8f76901` (все правки 22.09 выше), на проде `git pull` +
  `optimize:clear`, в `schedule:list` — `currency:fetch-rates` в 09:00. Перед догрузкой копия таблицы
  курсов — `/root/backup/currency_rates-20260922-131629.json` (2946 строк). Догружено от `www-root`:
  май 2025 и 01.05–22.09.2026, 176 дней без ошибок; пропусков с 01.01.2025 нет, курсы на 22.09 = ЦБ
  (USD 84,0954, SRD/SAR 22,4254). Через ядро под пользователем 1 — 200 без ошибок: «Китай» (курсы на
  21.09), окно валют воронки, платёжный календарь, реестр лицензий (Softline 34 / 45, ключей без
  партнёра 0), анализ скидок с двумя статусами, воронка.

## API OSMOVIEW CP: новый адрес (17.09.2026)

Алексей перенёс API: `https://cp-osm-702343414611.europe-west1.run.app` (ключ владелец передал
в чате; хранить только в `.env`). Проверено с рабочей машины и с прода: без ключа и с неверным
ключом — `401`, с ключом — `200` и JSON, редиректов нет. Обвязка AI Studio ушла.

**Синхронизацию пока запускать нельзя:** в `/api/cp/list` больше нет doc-id — поле `id` теперь
номер КП (`AK8-V1`), а `detail/{номер}` отвечает `404` (detail по-прежнему по doc-id, старые
doc-id работают). Номера в списке к тому же дублируются: 26 записей, 20 разных `id`. Если
запустить `external-proposal:sync` как есть, `upsertFromListRow()` запишет номер в `external_id`,
шесть КП склеятся с чужими записями, а detail по такому id не придёт. Просьба к Алексею вернуть
doc-id в список (полем `id` или отдельным `docId`) — дописана в конец
`patches/patch-v21/PROMPT_ALEXEY_API.md`.

Заодно выяснилось: номера КП в генераторе перенумерованы (`AK689` → `AK8-V1`), doc-id прежние —
из 27 записей портала API узнаёт 19, восьми больше нет. Список теперь отдаёт полные записи
(1,3 МБ на 26 КП), то есть после возврата doc-id портал сможет обходиться без запросов detail —
это правка `ExternalProposalService::sync()`.

## Дубли «партнёр = компания» и заказчиков (22.09.2026, обсуждение — правок нет)

Владелец: «партнёр и компания повторяются в некоторых случаях»: 1) МТ Интеграция должна остаться только
партнёром; 2) ЦОДД объединить, ЦОДД от LURE IT тоже в связку ЦОДД → МТИ; 3) сделки, висящие на МТИ как на
заказчике, разнести по ЦОДД и Метро. Разбор по данным прода (только чтение), скрипты — scratchpad
`cp_codd.php`, `cp_bitrix.php`, `cp_scan.php`, `cp_mti.php`, `cp_refs.php`.

**Портал, МТИ как заказчик — компания #869 «МТ Интеграция» (партнёр #839):**
- КП: PG611 (вагоны, won; сделка Б24 #369; спец С.1/С.2 Метро; КП обоих договоров) → Метро;
  PG671 (серийное производство под Метро) → Метро; PG675 (доп. работы, депо; спец С.2_Метро доп.) → Метро;
  PG631 «PoC — завод» и AK710 «Jetson Xavier» — заказчик неясен. У PG* партнёр КП — OSMOVIEW.
- Спецификации: 57, 58, 59, 82 (Метро) → Метро; 81, 83, 101, 108 (Лидары) → ЦОДД.
- Договоры #51 L-PG-303.25 и #52 S-PG-313.25: `company_id = 869`, внутри спецификации обоих заказчиков.
- Ключи: #121, #122 (лидары, С.3) → ЦОДД; #126 metro_thermal (С.1_Метро) → Метро. Проект #5 «Лидары.
  перенос точки» → ЦОДД.
**ЦОДД:** #42 (LURE IT, 2024-09-11; КП AA565 — партнёр КП OSMOVIEW, AK728/AK736/AK746 — партнёр КП МТИ;
2 заметки), #876 (МТИ; AK650, 15 редакций), #888 (МТИ, пустая). **Метро:** #968 «Метрополитен Москва»
(МТИ) создана 22.09.2026 13:15 пользователем 1, пустая; в Битриксе заказчик пишется «Метрополитен».
**Битрикс:** компания Б24 #303 «ГК «МТ-Интеграция»» (24 сделки) не сопоставлена с партнёром #839 —
реестр и вкладка партнёра МТИ их не видят. Заказчиком указана сама МТИ у #367 (демостенд), #369 (вагоны),
#433, #457 (оплата этапов) — правится только в Битрикс24 (у портала зеркало). Портал ищет компанию сделки
по точному названию заказчика: «Метрополитен» ≠ «Метрополитен Москва».
**Шире по базе:** 12 партнёров заведены компанией у самих себя (Билайн, NeuroView, Jet, Neirolis,
Телеком-Мастер, МТИ, test, АФС Борлас, Trafcoo, STARINCO, White Cirrus, Фалькон); 5 компаний OSMOVIEW
названы как партнёры (Полипластик, Татнефть, ТРК Одинцово, СТК, Ростелеком); дубль «Промстрой» (#929 СТК
пустая, #946 OSMOVIEW); 13 компаний Б24 похожи на партнёров, но не сопоставлены (Trafcoo 29 сделок,
Риверстарт 9, ПроЭнерджи 8, Телеком Мастер 5, …); у дилеров в Б24 ~25 сделок, где заказчик — сам дилер.
**Выполнено 22.09.2026 16:11** — решения владельца и итог в `patches/fix-companies-2026-09-22/README.md`
(сценарий `fix_companies.php`, копия базы `/root/backup/avgmom-2026-09-22_16-10.sql.gz`). PG631 остался
на #869, переименованной в «МТ Интеграция (внутренний запрос)» — выбор исполнителя по «оставим на МТИ как
их тестовый запрос». Код карточки компании (оплаты по спецификациям) — локально, ждёт выкатки. Проверка
сценария агентами (3 ракурса) зависла на ретраях и остановлена — проверено вручную. Две ошибки в
`laravel.log` прода 22.09 14:22 — от упавшего скрипта разведки, не сайта.
**Выкачено 22.09.2026:** коммит `87ca600` (карточка компании: оплаты по спецификациям заказчика + сценарий
и README правки). Проверено: карточки ЦОДД (оплат пришло 8, ожидается 2), Метро (10 / 0), внутреннего
запроса МТИ — 200. **Сбой при выкатке:** `optimize:clear` от root удалил `bootstrap/cache/packages.php` и
`services.php`, каталог принадлежит root, сайт (www-root) не смог их пересоздать — 500 с 16:14:32 примерно
на минуту (3 запроса в логе). Восстановлено `php artisan package:discover` от root. Впредь после
`optimize:clear` — `package:discover` и проверка `/auth`; насовсем — `chown www-root bootstrap/cache`
(решение владельца).
**22.09.2026 16:16:** по просьбе владельца `chown -R www-root bootstrap/cache` на проде. Проверено:
`runuser -u www-root -- php artisan optimize:clear` → сайт сам пересоздал `packages.php`/`services.php`
(владелец www-root), `/auth` — 200. Кеш на проде впредь чистить от www-root.

## Партнёры, заведённые заказчиками у себя — разбор (22.09.2026, правок нет)

16 карточек (11 «у себя» + 5 компаний OSMOVIEW с именем партнёра), данные прода — scratchpad `pc_dump.json`,
`pc_cases/`; разбор: свой + workflow `partner-as-customer-review` (4 аналитика + 4 скептика, без доступа к
проду). Предложение владельцу:
- **Ясные переносы:** STARINCO AK678 → Indorama (#900); СТК PG668 → МУП «Городские маршруты» Улан-Удэ,
  PG669 → БЦ Иваново, PG672 → ПК КУНДРАТ (новые у СТК, по сделкам Б24 #475, #503, #467); Ростелеком AM644
  → «Сыроварня» (новая у Ростелекома, сделка #451); JET AM777 → Данон (сделка #895; новая у Jet или #12
  OSMOVIEW?); Билайн AM621 → ЦУМ Барвиха (сделка #547 в день КП, medium).
- **Собственный запрос партнёра (оставить):** Trafcoo AK651 (демостенд, #469), Билайн OD454 (своя платформа,
  #111), NeuroView DS517 (#181, заказчик — сам партнёр); карточку JET — под сделку по вагонам #207.
- **Заказчик неизвестен:** Телеком-Мастер AM612 (#361), White Cirrus AK666 (#497, Испания), АФС Борлас AK638
  (КП старше карточки — перевешено), Neirolis AA497 (гипотеза — #65 «Птицефабрика»), СТК PG627/PG628
  (пилот птицефабрики, #399 без заказчика), PG662 «Сбермобайл», Trafcoo AK655 (или Amna #915).
- **Не дубли, оставить:** Татнефть (#39), ТРК Одинцово (#44), «мониторинг строительных работ (Фалькон)» (#940).
- **Мусор/пустые:** партнёр test + компания Test + AA670 «test 2» (PG617 «Норильск» Softline — спросить),
  Полипластик #17 (пустая); после переносов пустеют STARINCO #896, Ростелеком #886.
- Б24: пропущено сопоставление «Борлас АФС» ↔ АФС Борлас (порядок слов). Попутно: у ряда КП статус «в работе»,
  а сделки Б24 отменены (AK666, AM612, AM649, PG669, PG672 и др.).
**Повторная проверка 22.09.2026** (scratchpad `pc_recheck.php`, `pc_recheck2.php`; данные после выгрузки не
менялись). Новый сигнал — **совпадение сумм КП и сделок Б24 до рубля** (любая редакция/вариант, ±НДС):
подтверждены AK678↔#533, PG669↔#503, PG672↔#467, AM644↔#451, AK651↔#469, OD454↔#111, AM612↔#361 (с НДС),
AK666↔#497, PG628↔#399, OD757↔#195. **Исправления разбора:** AK655 = #479 Faris Al-Reem (6 976 USD —
скептик ошибся); AA497 (Neirolis) = #327 Телеком-Мастера «Птицефабрика (Osmoview)» → ООО Советская
агрофирма, 3 780 000, Completed (цель — #65 «Птицефабрика»); PG617 «Норильск» не тест — #385 Softline
«Норильск (Трубный цех)», та же сумма и день; AK638 ↔ #417 «Мониторинг строительной площадки Борлас»
(заказчик — сам Борлас); AM621 → ЦУМ Барвиха суммой не подтверждается (понижено); Данон #12 (OSMOVIEW) пуст —
можно передать Jet и положить туда AM777 без дубля. **Новое:** партнёры-дубли «T1» #25 (латиница, пустой) и
«Т1» #840 (4 компании, 4 КП); Б24 #247 «Т1 Интеграция» (6 сделок: Русал, ВСМПО…) не сопоставлена; явные
сопоставления Б24: #55 «Борлас АФС»→#842, #101 «1С:Северо-Запад»→#41, #437 «АДТ Секьюрити Солюшнз»→#845,
#247→#840. У ТРК #44 два КП «ТРК периметр» (AM738, OD757) на одной сделке #195 — возможный дубль. 17 КП, где
партнёр КП ≠ партнёр компании (Магазин/Билайн, OSIYOTECH/Techwizer, Киевская площадь/OSMOVIEW …) — отдельная тема.
**Вторая правка выполнена 23.09.2026 00:44** — `patches/fix-companies-2026-09-22/README.md` (раздел «Вторая
правка»), сценарий `fix_partner_customers.php`, копия `/root/backup/avgmom-2026-09-23_00-44.sql.gz`.
Проверено: карточки новых заказчиков, #65, #900, #56, партнёров 11, 21, 28, 842, 840, 13, списки компаний,
партнёров, КП, реестр сделок, скоринг — 200. Поправка к разбору: AM738 и OD757 ТРК — редакции одного КП (номер
сменили), не дубль. Причина заглушек — старые редакции КП остаются на прежней компании. Ждут: п.1 (пометки),
Борлас AK638, Билайн AM621, Данон; карточки «СТК» #56 и пустая Neirolis #864.
**23.09.2026:** White Cirrus → заказчик SINAPSYSTEC (компания #901 и AK666), выполнено. Предложение владельцу —
необязательная компания в КП (разбор в чате): `proposals.company_id` NOT NULL, `ProposalRequest` и перенос
внешних КП требуют компанию, ~10 мест выводят `->company->name` без проверки, сортировка списка КП по компании —
inner join. Предложено: компания необязательна, партнёр обязателен (по умолчанию OSMOVIEW), компания и партнёр —
свойство всего КП (правка в новой редакции переносится на все редакции) — это и есть причина заглушек.
**23.09.2026 00:57 — связи КП ↔ сделки и статусы по Битриксу** (`fix-companies-2026-09-22/link_deals_and_statuses.php`,
итоговая таблица — `link_deals_report.md`, копия базы `/root/backup/avgmom-2026-09-23_00-57.sql.gz`). Связь — только
при совпадении суммы до рубля (±НДС), партнёра (через partner_crm_companies; для OSMOVIEW — заказчик = компания),
даты ≤ 45 дней и единственности кандидата с обеих сторон: 49 новых связей, спорных 0. Статусы по стадиям
(выигрышные → won; Suspended → lost/frozen; Canceled → lost/canceled; по всем связям): 63 смены (57 → lost, 6 → won).
AK667 (En+, «Выиграно» по спецификации, сделка #507 отменена) не тронут. Партнёр White Cirrus удалён.
**Необязательная компания в КП** — код готов, проверен локально (17 страниц, API списка с сортировками, фильтр
«Заказчик не указан», 14 виджетов КП — 184 проверки), не выкачен: миграция `2026_09_23_100000`, ProposalRequest,
ProposalRepository (+ `syncCustomer()` — заказчик и партнёр на все редакции; сортировка по компании/партнёру —
`select('proposals.*')` + leftJoin), фильтр `noCompany`, формы создания/редактирования (пустой пункт «Заказчик не
указан», у партнёра «Без партнёра — выберите OSMOVIEW»), перенос внешних КП, имя PDF, быстрая заметка к КП.
**Выкачено 23.09.2026 01:03:** коммит `13e300b` (необязательная компания, фильтр «— Без компании —» в поле
«Компания» списка КП, сценарии правок данных). На проде: копия `/root/backup/avgmom-2026-09-23_01-02.sql.gz`,
правка владельца в `pub/proposal/index.blade.php` (`width: 1`) отложена `git stash` и возвращена после pull,
`migrate --force` (company_id NULL), `optimize:clear` от www-root. Проверено 16 страниц/запросов — 200 без ошибок,
в логе ошибок нет. Следующий шаг — убрать заказчика у КП на карточках-заглушках и удалить заглушки. Поиск в
списке КП медленный (~1,8 с на «683» локально): `getTable()` грузит все КП и режет страницу в PHP.
**23.09.2026 09:24 — сделки-транши и спецификации по аналогии с AK650** (`fix-companies-2026-09-22/link_tranches_and_specs.php`,
копия `/root/backup/avgmom-2026-09-23_09-24.sql.gz`). Модель AK650: КП ↔ спецификации ↔ платежи ↔ сделки-оплаты (суммы
лицензий/услуг сделки = платежи спецификаций; сумма спецификаций С.3_Лидары = КП до рубля). Поле
`uf_crm_1726842631129` — не «сумма КП» (почти везде небольшое число — видимо, камеры/лицензии). Связано 14 сделок:
OD588 ← #319/#321/#325 (BMW ч.2–4); AK736 ← #747/#751 (Тушино); PG675 ← #529; PG691 ← #589; AK734 ← #761; AK700 ← #607/#707
(Amna, суммы из спецификаций); AA595 ← #55 (Геленджик); AK732 ← #235, AK723 ← #221, AK727 ← #223 (РРПК 2024 г.).
Прикреплено 24 спецификации: С.3_Лидары → AK650; С.4 перенос точки → AK736; С.1 Маслосырзавод → PG691; S.1 Amna → AK700;
Геленджик (#50, #52) → AA595; #36/#37 → OD; НЛМК С.8–С.14 → PG568 (7 × 288 000 = КП); С.12 Механик Сизов → PG591;
С.7_Пулково → PG685; С.14_Березина → AK732; С.13_Владивосток → AK723; С.12_Павел Батов → AK727. Статусы: 6 КП → «Выиграно»
(AK736, PG691, AK700, AA595, OD, PG568); AA595 — сделка #55 Canceled, но «Выиграно» по спецификациям (оплачено 18,1 млн).
Пропущено по решению владельца: С.4 Пулково, Птицефабрика, En+, Русал, МТИ этапы, ВТБ PoC, продления клубов. Без КП остались
спецификации: Русал (13), En+ (3), Птицефабрика (2), ВТБ, Пулково С.4 (2), RASHMI (2), Крым (2), ГпПП, Олейничук, две без договора.
**23.09.2026 ~09:40 — новые КП и спецификации с прода в локальную базу + разбор связей.** Локальная база была копией
~11.09 и разошлась с продом: на проде 5 новых КП (AA793–AA797, 16–22.09), 2-е редакции продлений РРПК AA788–АА792
(это те же строки прода 817–821, переведённые во 2-е редакции групп AA781–AA785) и спецификации 109–111; локальные
строки 817–826 — наши тестовые переносы внешних КП (ВДНХ, Casi, Мираторг, «X»), id совпали случайно. Выгрузка с прода —
scratchpad `export_missing.php` → `missing_export.json`, импорт с перенумерацией — `import_missing.php` (компании 964–967
с теми же id, 10 редакций КП → локальные #829–#838 со всеми дочерними строками, спецификации 109–111, платежи,
привязки, 1 связь со сделкой, 2 записи внешних КП). После импорта групп «только на проде» — 0. Разбор связей: правила
link_deals_and_statuses на проде — новых точных совпадений 0; вручную — AA796 ↔ #865 «Аналитика дефектов автомобильных
дисков» (Softline, СКАД) — поставлено локально и на проде (`link_new_entities.php`, только добавление; копия
`/root/backup/avgmom-2026-09-23_09-39.sql.gz`; счётчики 10 ключевых таблиц до/после совпали, связей 126 → 127).
Не связаны: AA793 ВДНХ (дубль AK760, сделка #809 — демо), AA794 Киевская площадь (сделка #813 — OSMOVIEW, у AA778),
AA797 (сделки нет). Локальная база по-прежнему отстаёт от прода в остальном (связи, статусы, правки компаний).
**23.09.2026 09:53 — локальная база заменена копией прода.** Сравнение связей локально/прод по смысловым ключам:
только локально были PG611 ↔ #433/#457 (не в карточке КП, МТИ владелец пропустил), AM655 ↔ #763, проекты по сделкам
(24 шт., на проде удалены 15.09; #25/#26 — тесты), White Cirrus ↔ Б24, договор «111», тестовые переносы внешних КП —
владелец: «всё тестовое, не переноси». Копии прода `/root/backup/avgmom-2026-09-23_09-53.sql.gz` и
`avgbitrix-2026-09-23_09-53.sql.gz` (скачаны в `W:\Backup\MySQL\avg.mom\2026-09-23`), копии старой локальной базы там же
(`local-*-before-refresh-*`). После загрузки счётчики 13 таблиц и зеркала Битрикса совпадают с продом, миграций в очереди
нет. `backup_db.php` научился второму параметру — подключению (`bitrix` — зеркало Битрикса). Правила владельца в памяти:
свежесть локальной БД перед работой, копия прода с меткой времени перед любым изменением его базы.

**Следующий шаг — связка КП «главное / второстепенное»** (AA793 ↔ AK760): КП, заведённое в портале и пришедшее позже из
внешней системы, не склеиваются, а логически объединяются; только главное участвует в расчётах, скорингах и прочем,
второстепенное — только чтение и история; на карточке КП видно связку и что главное.

## Итерация 13 · patch-v33 · Связка КП «главное / второстепенное» (с 23.09.2026)

Запрос владельца: КП, заведённое в портале и пришедшее позже из внешней системы (AA793 ↔ AK760, ВДНХ), не
склеиваются, а логически объединяются. Только главное участвует в расчётах, скорингах и прочем; второстепенное —
только чтение и история; на карточке КП видно связку и что главное.

**Решения владельца (23.09.2026):**
- пара ВДНХ: **главное — AA793** (перенесено из OSMOVIEW CP, там AK528), второстепенное — AK760; главное можно менять;
- список КП: второстепенное **показывается приглушённо** с плашкой «второстепенное → AA793», у главного — значок связки;
- если у будущего второстепенного есть сделки Б24, спецификации или договоры — **связать не даём** (попап пишет, что
  мешает; привязки переносятся вручную обычными попапами). То же при смене главного: прежнее главное должно быть без
  привязок, иначе — разъединить, перенести, связать заново.

**Модель.** Таблица `proposal_links` (`main_group`, `secondary_group` UNIQUE, `comment`, `linked_by`, `linked_at`),
связка живёт на группах (все редакции). Цепочек нет: второстепенное не бывает главным, у главного нет главного. Связь
пишется в журнал обоих КП (дети `secondary_links` / `main_links` в `logChildren()`).

**Исключение из расчётов — явно, не глобальным scope** (разбор агентом 23.09: глобальный scope ломает привязку
маршрутов по `group` — ~30 маршрутов с 404, генерацию `number_int`/`iteration`, массовые update по группе,
`ProposalVariant->proposal` → падение карточки; а скоринг и топ сценариев — сырой SQL, их он всё равно не закрывает).
Scope `Proposal::counted()` и SQL-обрезок для сырых запросов в узких местах: `ProposalStatusService::latestIterations`
(через него счётчики, конверсия, MetricRegistry, ~10 виджетов), `MetricRegistry::groupsSentBetween`, `PipelineSumWidget`,
`DiscountAnalysisService::rows`, `CrmMismatchService`, `ReportService::scenario_popular`, `ChinaReportService`,
`SpecReconcileService` (сумма КП), `PartnerScoringService` (КП партнёров и привязки к спецификациям — сырой SQL),
`ScenariosTopWidget`, `PriceHistoryWidget` (выбор по умолчанию). Выборки для привязки (сделка, спецификация, договор,
проект) второстепенное не предлагают.

**Этапы (агентам, ≤5 файлов на задание):**
- A — миграция `proposal_links`, модель `ProposalLink`, связи и scope в `Proposal`, `ProposalLinkService`
  (link / unlink / makeMain / blockers / candidates / assertEditable);
- B — попап связки, API, карточка КП (плашка связки, баннер на второстепенном, скрытые правки);
- C — запрет правок второстепенного на сервере (редактирование, статус, сделки, спецификации, договоры, варианты,
  конвертация, удаление);
- D — исключение из расчётов (список выше) и из выборок для привязки;
- E — список КП (приглушённая строка, плашки), карточка компании (пометка, без сумм), сводная карточка сделки;
- затем проверка, README патча, коммит, выкатка (копия БД прода до `migrate`), связка ВДНХ на проде.

**Итог 23.09.2026.** Этапы A–E готовы (агенты), доработки мои: кнопки правки внутри вкладок варианта спрятаны у
второстепенного (`extra-pays`, `hardware_table`, `proposal_variant/task`), сумма в попапе — главный вариант (как в
списке), удалять нельзя только последнюю редакцию главного со второстепенными. Проверки: расчёты до/после связки
(КП в работе 105 → 104, сумма в работе −52 762 820, скоринг МТИ 14 → 13 КП, анализ скидок 202 → 201) — после отката
совпали с «до»; запреты — 39 пунктов; попап — 12; виджеты КП — 172 прогона `render_widgets.php`; прогон 21 страницы —
200. Локально пара ВДНХ связана (AA793 главное), журнал обоих КП со строкой связки. Замечено, не правилось: тексты 403
в попапах сделок/спецификаций не выводятся (обработчики не читают `message` — кнопки у второстепенного и так скрыты);
`PUT api.hardware.store` без `params` — 500 (было и до патча); `ReportService::scenario_popular()` содержит `dd($scenario)`
на сценарии без справочника; маршрут `proposal.edit_save` ведёт в несуществующий метод.
**Правки владельца по ходу (23.09.2026):** поиск в списке КП находит связку целиком (по «793» — AA793 и AK760;
`ProposalLinkService::clusterGroups()` в `getTable()`); «поиск очень не отзывчивый, ~4 с» — `scopeSearch()` переписан
на подзапросы `IN` (коррелированные whereHas по сценариям ~0,85 с × 2–3 запроса), запрос списка 2,5 с → 0,04 с,
результаты совпали со старым поиском на 9 запросах. Долг из записи 23.09 01:03 («поиск медленный, ~1,8 с») закрыт.
**Выкачено 23.09.2026 ~12:00:** коммиты `3d417a9` (правки владельца: ключ курсов, `width: 1`), `96e81fb` (сценарии связей
23.09, backup_db с подключением), `b353974` (v33). Прод на `b353974`, рабочая копия чистая (stash с правками владельца
совпал с `3d417a9` и удалён). Пара ВДНХ связана на проде: AA793 главное, AK760 второстепенное. Следом владелец
попросил ускорить поиск реестра сделок и страницу «Воронка продаж» — в работе у агента.
**23.09.2026, после выкатки — дерево в списке КП** (просьба владельца: «второстепенные в строчке основного, под ним,
как дерево»): `getTable()` берёт только `counted()`, `ProposalService::tableDefault()` вставляет второстепенные ветками
сразу после главного (одним запросом на страницу, метод строки `tableRow()`), ячейка «Название» рисует уголок
(`tree = child`), стили `osmo-tree-*` в `pub/proposal/index`. Поиск связок — по выборке с второстепенными
(`$builder_linked`), иначе «AK760» ничего не находил.
**23.09.2026 — ускорение «Воронки продаж»** (просьба владельца, агент): 12 компонентов страницы создавали по своему
`DashboardDataService` — теперь `DashboardDataService::shared()` (один на запрос при той же валюте и фильтре, WeakMap по
объекту запроса), курсы один раз на экземпляр, `filter()` по ключу вместо `contains()`, `industry_name()` запоминается,
N+1 в четырёх таблицах и проверке проблемных сделок (`CrmDealRepository`) — пачкой. 1,6–2,1 с / 803 SQL → 0,3–0,4 с / 72 SQL;
HTML и данные всех методов совпали побайтно, 9 попапов по md5, виджеты воронки — 124 прогона `render_widgets.php` без ошибок.
Реестр сделок на сервере 80–150 мс (как пустая страница) — задержку даёт браузер: поиск через 0,7 с после ввода и
перезагрузка всей страницы. **На проде включён debugbar** (`APP_DEBUG=true`, `APP_ENV=development`) — раздувает каждую
страницу и показывает SQL/сессию; предложено выключить (решение владельца).
**23.09.2026 — реестр сделок без перезагрузки и отладка на проде** (решение владельца): поиск реестра запрашивает
`?…&partial=1` — `CrmDealController::index` отдаёт JSON (таблица `_table`, кнопки `_filter_buttons`, адрес выгрузки),
страница подменяет `#deal_table_wrap`, `#deal_filter_buttons`, `q` в форме фильтра и ссылку `#deal_export`, адрес — через
`history.replaceState`; пауза ввода 700 → 300 мс. От ввода до таблицы ~0,53 с (было ~1,5 с с перезагрузкой). На проде
`APP_DEBUG=false` (копия `.env.before-debug-off-2026-09-23`), debugbar в страницах 25 → 0, 404 без трассировки;
`APP_ENV=development` и `LOG_LEVEL=debug` не трогались. В `storage/debugbar` прода 1153 файла (86 МБ) — не удалялись.
Снимки debugbar на проде (1151 файл, 86 МБ, SQL и сессии) по просьбе владельца убраны из `storage/debugbar` —
сначала упакованы в `/root/backup/debugbar-2026-09-23_12-25.tar.gz` (5,4 МБ, 0600, число файлов в архиве сверено);
`.gitignore` каталога оставлен. Архив владелец может удалить сам.

## Итерация 14 · patch-v34 · Рабочий стол: задачи, календарь, стартовые столы, аудит 80 виджетов (23.09.2026)

Полное описание, файлы, выкатка и чек-лист — **`patches/patch-v34/README.md`**; итоги аудита по группам —
`patches/patch-v30/grid/{группа}.md` (раздел «Аудит 23.09.2026» у каждого виджета).

**Запросы владельца по ходу (все сделаны):** блокнот → задачи (флажок «сделано», `user_notes.done_at`,
`api.user-notes.done`; правка заметки больше не пересоздаёт её); календарь «как в Windows 11» (листание,
плитки месяцев/лет, кружки дней с событиями, панель дня, «+ Новое событие» с датой); выравнивание описания
в блокноте; «Курсы валют» 8×3 и сломанный значок (подключён **FA Pro 6.0.0** — значков 6.1+ нет, заменены 7);
блок высотой 1 — без заголовка; сайдбар события в Metronic (`themes/metronic/pub/calendar/sidebars/_form`);
одинаковые отступы у «Действия»; «разработай недостающие виджеты» — каталог 80 закрыт, владелец выбрал
только **стартовые столы**; аудит всех 80 (11 агентов по группам) — вёрстка без X/C/H/F/G, неточности в данных.

**Механизмы ядра стола (новые, пользоваться ими):** состояние просмотра блока `Widget::viewState()` +
`[data-desk-view='{json}']` (третий аргумент `data()`/`sample()`, `$view` во вьюхе, стенд `--view=`);
действие без перехода `[data-desk-post="url"]`; `$(document).trigger('desk:refresh', ['id'])` из сайдбаров;
`data-fit-extra` у «ещё {n}»; `.min-w-0`; несколько `.desk-cut` в строке делят ширину; графики берут `#rrggbb`.

**Решения владельца 23.09 (сделаны):** дата выигрыша КП = первая фактическая оплата привязанной
спецификации, иначе `status_changed_at` (`MetricRegistry::wonDates()`/`decidedDates()`); сравнение «к прошлому»
для фактических показателей — по то же число (`DesktopContext::previousRange($key, $now, $to_date)`),
прогнозные — целиком; `Execution (PRE-PAYMENT)` — завершённая сделка (оплата прошла); сумма спецификации
делится между её ключами (`LicenseRegistryService::decorate()`, «1/N спецификации» в реестре);
саудовский риял — код `SAR` (миграция, `currency:fetch-rates` без псевдонима); КП AM516 «НГМК» закрыто
(«Проиграно»/«Отменено»); воркер очереди на проде настроен.

**Прод (выкачено 23.09 ~15:22, коммиты `ede25df`, `90caea7`, `8d6ef44`):** копии
`/root/backup/avgmom-2026-09-23_15-11.sql.gz` (до правок данных) и `…_15-21.sql.gz` (до миграций);
supervisor `/etc/supervisor/conf.d/osmo.conf` — программа `osmo_queue` (www-root, `--queue=database,default`),
после выкатки кода — `php artisan queue:restart`; 3 тестовых напоминания Анны 2024 года сняты из `jobs`;
пресеты #3–#7 («Руководитель» — по умолчанию). **Инцидент: 500 с 15:33 до 15:36** — проверочный скрипт от
root создал шаблоны в `storage/framework/views`, сайт (www-root) не мог их перезаписать; исправлено
`chown -R www-root storage bootstrap/cache`. **Правило: на проде всё, что рендерит или пишет кэш, — от
www-root** (`su www-root -s /bin/sh -c "php …"`), проверка — `patches/patch-v34/tools/prod_check.php`.

**Открыто (ждёт владельца):**
1. На проде `APP_ENV=development` → `EmailNotification` шлёт **все письма напоминаний на avg.den@yandex.ru**;
   с работающим воркером они пойдут туда. Нужно ли `APP_ENV=production`.
2. Конверсия КП перекошена: выигрыши датируются оплатами, а все 71 проигрыш — пачкой `status_changed_at` в
   III кв. 2026 (конверсия за квартал 11,3 %). Варианты — в README v34.
3. МТИ «Метро» (L-PG-303.25): платформа с ключом 126 + нейронки без ключа — один проект; нужна явная
   привязка ключа к нескольким спецификациям (автоматически нельзя — у РРПК спецификация = корабль).
4. Suspended-сделки в «Сделках по кварталам» — больше половины суммы в «Все, кроме отменённых».
5. Стол #8 у пользователя 5 (Алексей Карасев) — копию «Руководителя» создала моя проверка при заходе под ним
   (то же случилось бы при его первом входе). Удалить или оставить — спросить.
6. Мелочи из аудита: заголовок «Популярных сценариев» ведёт на справочник (отчёт выключен в меню); цвета
   грейдов Agent и Vendor оба `#000`; стрелка/фильтр грейда в scoring_top; ссылка specs_status.
7. Вне стола (не правилось): `ReportSpecService::setActive()` — не та колонка (галочки «активна» на
   `/report/specs` не сохраняются); выгрузка оплат с листом и именем файла как у конфигураций;
   130 выгрузок в `storage/app/public/temp` на проде (по ссылке не открываются, 404).

**Локальная БД** после работы совпадает с продом по этим правкам (миграции, AM516, пресеты #3–#7); у
пользователя 5 локально копии стола нет.

## Итерация 15 · patch-v35 · Аудит и правка дизайна темы Metronic (25.09.2026)

Запрос владельца: «с помощью /frontend-design сделай аудит и правку дизайна», «улучшай только новый шаблон
metronic»; макет PDF из КП входит в аудит. Полное описание, дизайн-план, файлы, выкатка, чек-лист и решения
владельца — **`patches/patch-v35/README.md`**; стенд аудита — `patches/patch-v35/tools/` (Playwright + WebKit,
каталог `shots` из WIKI).

**Главное:** новый слой `public/metronic/css/osmo-design.css` (подключён последним в `layout_short`): текст
статусов и серый — «чернилами» ≥ 4.5:1 (заливки кнопок и плашек не менялись), тёмная тема без белых пятен,
табличные цифры (кроме `#desk_grid`), фокус с клавиатуры, reduced motion. Точечно: макет PDF (жёлтые строки
1.7 → 6.8:1, отступ последней колонки, печать фона, лист всегда светлый), колонка вариантов КП 150 px,
прокрутка таблиц воронки и карточки компании, лента напоминаний и панель календаря (`osmo-compat-pages.css`),
пустое состояние уведомлений, колонки карточек партнёра/компании на телефоне, `/auth` без 500.

**Замер (текст ниже 4.5:1, до → после):** светлая 1440 — 3951 → 223 (40 страниц); тёмная — 2387 → 86
(14 страниц, замеренных до патча; по всем 40 после — 187); телефон — 1612 → 89 (8 страниц). Белых блоков
в тёмной теме не осталось. Попутно найдено: `AjaxApi` без сессии зовёт несуществующий
`UserRepository::getByToken()` → 500 (в v35 не правилось, исправлено в v36).

**Ждёт владельца** (список с вариантами — README v35): заливка `btn-primary` (3.6:1), один цвет главного
действия (синий vs фиолетовый `btn-info`), стол на экране меньше 1800 px, единый формат сумм («69.04M» / «11,1 млн $»),
красный грейд E у 45 из 47 партнёров, прозрачные ступени `palette.css` на плашках.

Синхронизация Битрикс24 через BI-коннектор (раньше планировалась как v35, потом v36) — теперь **v37**.

## Итерация 16 · patch-v36 · AJAX API без сессии (25.09.2026)

Найдено в аудите v35: `app/Http/Middleware/AjaxApi.php` для запроса без сессии звал несуществующий
`UserRepository::getByToken()` → 500 на любом AJAX-запросе гостя или с истёкшей сессией. **Решение
владельца 25.09:** вход по одному `ajax_token` не нужен (токен в адресах запросов и живёт до выхода —
стал бы бессрочным ключом). Без сессии middleware отвечает `{error: 'auth'}` (200, как при чужом токене) —
по нему пульс `spider.tick` уводит на форму входа. Проверено локально (curl и WebKit). В том же патче три
обработчика, показывавшие «Это успех!» без проверки ответа (`work_calendar` ×2, `analytics_bind`, тема
Metronic), на `{error: 'auth'}` показывают «Сессия истекла». Обе страницы мёртвые (нет модели
`UserWorkCalendar`, вычищены `LabObject`/`LabMeasure`, в меню их нет) — правку владелец решил оставить,
удаление страниц — отдельная задача.

Нумерация: v36 — эта правка (решение владельца), синхронизация Битрикса — v37.

## Итерация 17 · patch-v38 · Мёртвые страницы модуля User (25.09.2026)

«Рабочий график» (`/user/{id}/work_calendar`, 500 — моделей `UserWorkCalendar`/`WorkCalendar` нет с первого
коммита) и «Привязка аналитика к объектам» (`/user/analytics/{id}`, 403 — права `lab_objects_bind` нет; за ним
вычищенные `LabObject`/`LabMeasure`) — остатки прошлого проекта, в меню их нет, таблиц в базе нет. **Решение
владельца 25.09:** удалить страницы, мёртвые импорты в трёх сервисах и осиротевшие шаблоны `lab`; файлы — по
одному с подтверждением. Итог: 7 маршрутов, 18 файлов, точечные правки в контроллерах, `UserService`, `User`.
`route:list` 361 → 354. Снимает правку обработчиков v36 в двух удалённых шаблонах. Попутно найдено:
`/api/user/list_table` — 500 (`User::groups()` убрана с кластером, `UserRepository` её ещё зовёт) — отдельная задача.

## Итерация 18 · patch-v39 · Список пользователей без групп и подразделений (25.09.2026)

Найдено в v38: `UserRepository::getTable()` фильтровал по `group_id` / `department_id` и грузил
`withCount` / `with` связей `groups` и `departments`, убранных вместе с `UserGroup` / `UserDepartment` →
`/api/user/list_table` 500, таблицы «Пользователи» (`/user/list`) и «Доступы → Персональные» пустые.
Фильтры и счётчики убраны точечно; шаблоны и их JS этих полей не читают. Обе страницы рендерятся через
`pub::` — в обеих темах рисуются шаблоны из `resources/views/pub/user/`, копия в `themes/metronic` не
используется и не правилась. В этих двух шаблонах ожившая таблица показала «null» вместо пустого отчества —
`(row.second_name || '')`. Проверено: API 200 (7 строк, поиск, пагинация), таблица в браузере в обеих темах.
Попутно: `/access/set/user` — 403 всем, права `access_set` нет в `accesses` ни локально, ни на проде;
неизвестный `sort` → 500 (со страницы не воспроизводится). Подробности — `patches/patch-v39/README.md`.

**Выкачен на прод 25.09.2026** (`97123d4` → `547381a`): `git pull`, `view:clear` и `queue:restart` от www-root.
На проде до выкатки `tableDefault` падал на `User::groups()`, после — 7 из 7 пользователей, поиск работает;
`laravel.log` чистый. Пустой каталог кэша `storage/framework/cache/data/4a/e4` с владельцем root (создан в 09:00 —
cron `schedule:run` от root) отдан www-root.
