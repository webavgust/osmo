...
| 16 | v39 | Список пользователей: `/api/user/list_table` 500 (`User::groups()`) → 200 — из `UserRepository::getTable()` убраны фильтр и счётчики групп/подразделений; «null» вместо отчества в двух шаблонах `pub/user` | закоммичен 25.09.2026, на прод не выкачен; README `patches/patch-v39/README.md` |
...
Попутно: `/access/set/user` — 403 всем, права `access_set` нет в локальной `accesses` (прод — проверить);
