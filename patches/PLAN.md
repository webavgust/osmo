...
| 12 | v34 | Рабочий стол: задачи в блокноте, календарь как в Windows 11, сайдбар события, стартовые столы (5 пресетов, `desktop:presets`), аудит всех 80 виджетов | готов 23.09.2026, не закоммичен, на прод не выкачен; README `patches/patch-v34/README.md` (выкатка: миграция `user_notes.done_at`, `desktop:presets`, `cache:clear`; открытые решения владельца — там же) |
