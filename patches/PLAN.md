...
| 13 | v35 | Аудит и правка дизайна темы Metronic: 43 страницы (включая макет PDF из КП) в 5 режимах — читаемость текста («чернила»), тёмная тема, широкие таблицы, макет PDF, напоминания/календарь/уведомления, телефон | готов 25.09.2026, не закоммичен; README `patches/patch-v35/README.md` (решения владельца — там же) |
| 14 | v36 | AJAX API без сессии: `ajax.api` отвечает `{error: 'auth'}` вместо 500 (сломанный вход по токену убран, решение владельца) | готов 25.09.2026, не закоммичен; README `patches/patch-v36/README.md` |
| 15 | v38 | Удалены мёртвые страницы «Рабочий график» и «Привязка аналитика к объектам» (7 маршрутов, 18 файлов, точечно — контроллеры, сервис, модель User); v37 — за Битриксом | готов 25.09.2026, не закоммичен; README `patches/patch-v38/README.md` |
