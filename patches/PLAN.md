...
- **Курсы валют: показывался самый старый курс, а на проде курсы не обновлялись (22.09.2026).**
  ...
  **На проде ещё не сделано:** выкатка и догрузка
  `php artisan currency:fetch-rates --from=2026-05-14 --to=2026-09-22` (плюс май 2025) — после выкатки,
  иначе SRD не загрузится. Ключ exchangerate-api зашит в `CurrencyService.php` (правка владельца,
  не закоммичена) — после перехода на ЦБ он не нужен.
