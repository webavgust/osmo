...
|---|------|------------|-----------|
| 1 | v21 | API Алексея (OSMOVIEW CP): забирать КП и переносить к себе (ТЗ п.2) | готов 2026-09-09, проверен локально, ждёт коммита; API закрыт авторизацией AI Studio — работает импорт JSON |
| 2 | v22 | Реестр сделок Bitrix + выгрузка в Excel (ТЗ п.1) | готов 2026-09-09, проверен локально, ждёт коммита |
| 3 | v23 | Партнёры ↔ компании Bitrix, вкладки на карточке партнёра, вкладка «Сделки Битрикс» (ТЗ п.3, часть 1) | часть 1 (сопоставление, форма, блок на карточке) готова 2026-09-09; часть 2 (табы + вкладка «Сделки Битрикс» на базе `bitrix/deal/_filter`, `_table`) — не начата |
| 4 | v24 | Сущность «Проект» по сделкам, вкладки «Проекты» / «Архив проектов», разовое автосоздание (ТЗ п.3, часть 2) | не начато |
| 5 | v25 | Значок проекта у сделок по всему сайту (п.4) + список КП без перезагрузки (п.6) | не начато |
| 6 | v26 | Скоринг партнёров: сделки и проекты вместо «КП за год», новые веса (п.5) | не начато |
...
«пилот» → `info`; во вкладке партнёра у сделки без проекта — кнопка
«+ проект». Режимы `mode=projects` (сделки с активным проектом) и
`mode=archive` (проект в архиве) — вкладки «Проекты» и «Архив проектов» на
карточке партнёра.
