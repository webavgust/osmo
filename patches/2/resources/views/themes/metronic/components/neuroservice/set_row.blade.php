...
<x-ui.inputs.radio_group_neuroservice :group="$group" :neuroservice="$neuroservice" checked="{{$checked ?? 0}}"  ></x-ui.inputs.radio_group_neuroservice>

<div class="ms-3 content-todo">
    <h5 class="fw-semibold fs-4 todo-header pt-1" data-todo-header="Meeting with Mr.Jojo Sukla at 5.00PM">
        {{ $neuroservice->name }}
    </h5>
    @if($neuroservice->description)
