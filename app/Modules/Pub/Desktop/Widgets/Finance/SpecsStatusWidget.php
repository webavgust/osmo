 ...
 * все спецификации портала, любого типа договора и организации. Год берётся по дате
 * спецификации, а если своей даты нет — по дате рамочного договора (как у ContractSpecification::date).
 *
 * Страницы со спецификациями по статусам в портале нет; ссылка ведёт на отчёт
 * «Конфигурации. Сводная» — он показывает только лицензионные спецификации без отменённых,
 * поэтому цифры в нём меньше, чем на виджете.
 *
...
class SpecsStatusWidget extends Widget
{
    ...
    {
        // ближайшая страница о спецификациях: отчёт «Конфигурации. Сводная» (только лицензии,
        // без отменённых — см. описание класса); отбор отчёта в адрес не переносится
