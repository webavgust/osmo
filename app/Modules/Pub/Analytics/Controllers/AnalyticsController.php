...
class AnalyticsController extends Controller
{
     ...
     *
     * Один попап с закладками: разбивка по годам и расшифровки колонок — КП,
     * объёма, договоров, платежей, сделок Битрикса и проектов (patch v26).
     * Из таблицы каждая цифра открывает свою.
     *
     * @param Request $request
     * @param Partner $partner
     * @param string $tab stats|proposals|volume|contracts|payments|deals|projects
     * @return \Illuminate\Contracts\View\View
     */
    public function box_partner(Request $request, Partner $partner, string $tab = 'stats')
    {
        ...
        return View::make('pub.analytics.boxes.partner', [
            ...
            'proposals' => PartnerStatsService::proposals((int) $partner->id, $year),
            ...
            'deals' => PartnerStatsService::deals((int) $partner->id, $year),
            'projects' => PartnerStatsService::projects((int) $partner->id, $year),
            'crm_linked' => PartnerStatsService::crmLinked((int) $partner->id),
            'statuses' => ProposalStatus::getDecorated(),
        ]);
    }
}
