...
class SoftwareListFilterService
{
    private $filter;
    private $token;
    ...
    public function __construct($token = false)
    {
        ...
        $this->token = $token;
        $this->filter = Cache::get('software.list.filter:' . $this->token);

        # INIT FIRST FILTER STATE
        if (empty($this->filter) && !Session::get('is_ajax') && !Session::get('software.list.filter.init')) {
    ...
    public function setFilter($arFilter)
    {
        ...
        Cache::set('software.list.filter:' . $this->token, $this->filter, 86400);
    ...
    {
        Cache::forget('software.list.filter:' . $this->token);
    }
