...
class WorkListFilterService
{
    ...
    public function __construct($token = false)
    {
        ...
        $this->token = $token;
        $this->filter = Cache::get('work.list.filter:' . $this->token);

        # INIT FIRST FILTER STATE
        if (empty($this->filter) && !Session::get('is_ajax') && !Session::get('work.list.filter.init')) {
    ...
    public function setFilter($arFilter)
    {
        ...
        Cache::set('work.list.filter:' . $this->token, $this->filter, 86400);
    ...
    {
        Cache::forget('work.list.filter:' . $this->token);
    }
