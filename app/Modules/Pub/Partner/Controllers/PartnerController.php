...
class PartnerController extends Controller
{
    ...
    /**
     * Отбор вкладок со сделками по умолчанию.
     *
     * На карточке партнёра «есть КП» не важно: тут смотрят на всю работу с
     * партнёром, а не выискивают непосчитанные сделки, как в реестре
     * (правка владельца 11.09.2026).
     */
    public const TAB_DEFAULTS = ['has_proposal' => 'all'];
    ...
    public function detail(Request $request, Partner $partner = null)
    {
        ...
        return view('pub.partner.detail', [
            ...
            // умолчаниями, с какими вкладка откроется (patch v25)
            'deal_counts' => CrmDealRegistryService::counts($partner, static::TAB_DEFAULTS),
        ]);
    }
    ...
    public function deals(Request $request, Partner $partner)
    {
        ...
        $defaults = array_merge(CrmDealRegistryService::modeDefaults($mode), static::TAB_DEFAULTS);
        $params = CrmDealRegistryService::params($request, $defaults);

        return view('bitrix.deal._tab', array_merge(CrmDealRegistryService::options($partner), [
            'params' => $params,
            'defaults' => $defaults,
