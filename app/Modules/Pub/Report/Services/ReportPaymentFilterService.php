...
class ReportPaymentFilterService
{
    private $filter;
    private $token;
    ...
    public function __construct($token = false)
    {
        ...
        $this->token = $token;
        $this->filter = Cache::get('report_payments.list.filter:' . $this->token);


        # INIT FIRST FILTER STATE
        if (empty($this->filter) && !Session::get('is_ajax') && !Session::get('report_payments.list.filter.init')) {
    ...
    public function setFilter($arFilter)
    {
        foreach($arFilter as $field => $value) {
            ...
            if(is_array($value) && isset($value['from']) && empty($value['from']) && empty($value['to']))
                unset($arFilter[$field]);
        }

        $this->filter = $arFilter;
        Cache::set('report_payments.list.filter:' . $this->token, $this->filter, 86400);
    ...
    {
        Cache::forget('report_payments.list.filter:' . $this->token);
    }
