...
class DealChainService
{
    ...
    public static function build(Proposal $proposal): array
    {
        ...
        $specs = static::specifications($contracts->pluck('id'), $proposal->group);
    ...
    /**
     * Спецификации, прикреплённые к этому КП.
     *
     * У договора спецификаций обычно много, и почти все они относятся к другим
     * сделкам того же партнёра. В карточке нужны только те, что прикреплены к
     * группе КП — иначе деньги и шаги цепочки считаются по чужим спецификациям
     * (правка владельца 11.09.2026).
     *
     * @param \Illuminate\Support\Collection $contractIds
     * @param string|null $proposalGroup Группа КП; без неё вернутся все спецификации договоров
     * @return \Illuminate\Support\Collection
     */
    public static function specifications($contractIds, $proposalGroup = null)
    {
        ...
        return collect(DB::table('contract_specifications as s')
            ...
            ->when($proposalGroup, fn($builder) => $builder->whereIn('s.id', function ($query) use ($proposalGroup) {
                $query->select('contract_specification_id')
                    ->from('contract_specification_proposals')
...
//        $live_contracts = $contracts->where('old', '!=', 1);
        ...
        return [
            ...
            [
                ...
                'icon' => 'fa-handshake',
                // сумма сделок не сошлась с последним вариантом КП — это ошибка,
                // а не «шаг пройден»
                'state' => match (true) {
                    $links->isEmpty() => 'empty',
                    !empty($check['has_errors']) => 'warn',
                    default => 'ok',
                },
                'value' => match (true) {
