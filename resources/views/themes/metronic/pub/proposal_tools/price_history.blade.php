...
@section('content')
    @php
        ...
        $from_currency = $rows->firstWhere('iteration', $from)['currency'] ?? $last['currency'];
        $to_currency = $rows->firstWhere('iteration', $to)['currency'] ?? $last['currency'];
    @endphp
                ...
                <div class="row g-5">
                    <div class="col-6 col-lg-3">
                        ...
                        <div class="fs-3 fw-bold">
                            {{ tools()->cost_normalize(round($first['total'])) }}
                            <span class="fs-6 text-muted">{{ $first['currency'] }}</span>
                        </div>
                    ...
                    <div class="col-6 col-lg-3">
                        ...
                        <div class="fs-3 fw-bold">
                            {{ tools()->cost_normalize(round($last['total'])) }}
                            <span class="fs-6 text-muted">{{ $last['currency'] }}</span>
                        </div>
                        <div class="fs-8 text-muted">
                    ...
                    <div class="col-6 col-lg-3">
                        ...
                        @if($first['currency'] !== $last['currency'])
                            <div class="fs-8 text-danger">
                                валюта менялась: {{ $first['currency'] }} → {{ $last['currency'] }}
                            </div>
                        @endif
                    </div>
                    <div class="col-6 col-lg-3">
            ...
            <div class="card-header min-h-auto py-5 border-bottom">
                <div class="card-title flex-column align-items-start">
                    ...
                    <span class="text-muted fs-7">
                        Суммы основного варианта каждой редакции, в валюте редакции без пересчёта.
                    ...
                    <table class="table table-row-dashed table-row-gray-300 align-middle mb-0">
                        ...
                        @foreach($rows as $row)
                            <tr @class(['bg-light-primary' => $row['iteration'] === $last['iteration']])>
                                ...
                                <td>
                                    <span @class(['fs-7', 'text-danger fw-semibold' => $row['currency'] !== $last['currency']])>
                                        {{ $row['currency'] }}
                                    </span>
                                </td>
                    ...
                    <div class="table-responsive">
                        <table class="table table-row-dashed table-row-gray-300 align-middle mb-0">
                            ...
                            <tr class="fw-bold text-muted bg-light">
                                ...
                                <th>Позиция</th>
                                <th class="text-end" width="180">Редакция {{ $from }}, {{ $from_currency }}</th>
                                <th class="text-end" width="180">Редакция {{ $to }}, {{ $to_currency }}</th>
                            ...
                            @foreach($diff as $item)
                                ...
                                <tr @class(['opacity-75' => $item['state'] === 'same'])>
                                    ...
                                    <td class="text-end">
                                        @if($item['from'])
                                            <div class="fw-semibold">
                                                {{ tools()->cost_normalize(round($item['from']['total'])) }}
                                                <span class="text-muted fs-8">{{ $from_currency }}</span>
                                            </div>
                                            <div class="fs-8 text-muted">
                                    ...
                                    <td class="text-end">
                                        @if($item['to'])
                                            <div class="fw-semibold">
                                                {{ tools()->cost_normalize(round($item['to']['total'])) }}
                                                <span class="text-muted fs-8">{{ $to_currency }}</span>
                                            </div>
                                            <div class="fs-8 text-muted">
                            ...
                            <tr class="fw-bold border-top border-gray-300">
                                ...
                                <td class="text-end text-nowrap">
                                    ...
                                    <span class="text-muted fs-8 fw-normal ms-1">{{ $from_currency }}</span>
                                </td>
                                <td class="text-end text-nowrap">
                                    {{ tools()->cost_normalize(round($diff_total['to'])) }}
                                    <span class="text-muted fs-8 fw-normal ms-1">{{ $to_currency }}</span>
                                </td>
                                ...
                                <td class="pe-5">
                                    @if($from_currency !== $to_currency)
                                        <span class="badge badge-light-danger">разные валюты</span>
                                    @endif
                                </td>
                            </tr>
                            </tfoot>
