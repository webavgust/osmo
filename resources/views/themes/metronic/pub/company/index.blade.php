                ...
                <div class="card">
                    <div class="card-body pt-2">
                        <table class="table"
                               ...
                               data-url="{{ route("api.companies.list_table", ['_token' => auth()->user()->ajax_token]) }}"
                        ></table>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="container">
                            ...
                            <div class="mt-4 row">
                                <label class="col-sm-3 col-form-label fw-semibold text-lg-end">Сектор</label>
                                <div class="col-sm-9">
                                    <x-ui.select.single class="select2" name="sector" required :items="$sectors" id="id" value-name="label" :value="$filter['sector'] ?? null"></x-ui.select.single>
                                </div>
                            </div>
                            <div class="mt-4 row">
                                <label class="col-sm-3 col-form-label fw-semibold text-lg-end">Партнёр</label>
                                <div class="col-sm-9">
...
@section('js')
    ...
    <script>
        ...
        function filter_remove() {
            $.ajax({
                url: '{{ route('api.companies.filter.remove', ['_token' => auth()->user()->ajax_token]) }}',
                method: 'get',
                dataType: 'json',
        ...
        function filter() {
            $.ajax({
                url: '{{ route('api.companies.filter', ['_token' => auth()->user()->ajax_token]) }}',
                method: 'post',
                dataType: 'json',
        ...
        function IDFormatter(value, row) {
            if(value) {
                @can('education_application_view')
                    return '<a href="{{ route('company.detail') }}/' + row.id+ '">' + row.id + '</a>';
                @else
                    return row.id;
                @endcan
            } else {
        ...
        function sectorFormatter(value, row) {
                ...
                <div class="dropdown-action">
                    <div class="dropdown todo-action-dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-right">
                            <a class="dropdown-item" href="{{ route('company.edit') }}/` + row.id + `">
        ...
        function row_delete(id) {
            ...
            $.ajax({
                url: "{{ route('api.company.delete') }}/" + id + "?_token=" + csrf_token(),
                type: "DELETE",
                dataType: "json",
                success: function (response) {
                    if (response.result == 'success') {
