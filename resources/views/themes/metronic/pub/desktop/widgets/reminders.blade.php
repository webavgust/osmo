...
@php
    ...
    // сводка — полные числа из data() (список режет «Сколько показывать»); null — просроченные выключены
    $overdue = array_key_exists('overdue', $data) ? $data['overdue'] : count(array_filter($data['rows'], fn($row) => $row['overdue']));
    $ahead = $data['ahead'] ?? count($data['rows']) - (int) $overdue;

    // пустое состояние — с горизонтом: дальше него напоминания могут быть
    $empty = [
        'today' => 'На сегодня напоминаний нет',
        '3days' => 'На 3 дня напоминаний нет',
        'week' => 'На неделю напоминаний нет',
    ][$settings['horizon']] ?? 'Напоминаний нет';
@endphp
@if(empty($data['rows']))
    {{-- в узком блоке слово шире тела и выталкивает значок — там только значок, текст в title --}}
    <div class="desk-empty" title="{{ $empty }}">
        <i class="fa-light fa-alarm-clock"></i><span class="desk-hide-narrow">{{ $empty }}</span>
    </div>
@else
    <div class="desk-stack">
        ...
        <ul class="desk-list rem-list desk-stack-grow desk-fit">
            @foreach($visible as $row)
                @php
                    $href = $preview || empty($row['url']) || !empty($row['sidebar']) ? 'javascript:void(0)' : $row['url'];
                    // событие календаря и другие объекты-сайдбары открываются сайдбаром, как на странице напоминаний
                    $open = !$preview && !empty($row['sidebar']) ? "sidebar({href: '" . $row['sidebar'] . "'})" : null;
                ...
                <li>
                    ...
                    @if($roomy)
                        <div class="desk-grow">
                            <a href="{{ $href }}" @if($open) onclick="{{ $open }}" @endif class="d-block desk-nowrap text-reset" title="{{ $hint }}">{{ $row['text'] }}</a>
                            @if(!empty($row['object']))
                                <div class="desk-muted desk-nowrap fs-7" title="{{ $hint }}">{{ $row['object'] }}</div>
                            @endif
                        </div>
                    @else
                        <a href="{{ $href }}" @if($open) onclick="{{ $open }}" @endif class="desk-grow text-reset" title="{{ $hint }}">{{ $row['text'] }}</a>
                        @if(!empty($row['object']))
