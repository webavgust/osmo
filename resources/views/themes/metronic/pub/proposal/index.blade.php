...
@section('js')
    ...
    <script>
        ...
        /**
         * Перерисовать ячейку «Сделка» после привязки или отвязки (patch v25).
         *
         * Список пагинируется на сервере, поэтому перезагрузка страницы
         * сбрасывала бы и страницу, и сортировку. Меняем ровно одну ячейку —
         * ту, что помечена data-deal-cell группой КП.
         *
         * @param {string} group группа КП
         * @return {void}
         */
        window.deal_changed = function (group) {
            var $cell = $('[data-deal-cell="' + group + '"]');
            if (!$cell.length) return;

            $.ajax({
                url: '{{ route('api.proposal.deal_cell', ['proposal' => '__group__', '_token' => auth()->user()->ajax_token]) }}'.replace('__group__', encodeURIComponent(group)),
                type: 'GET',
                dataType: 'json',
                success: function (response) {
                    if (response.result !== 'success' || !response.html) return;

                    $cell.replaceWith(response.html);
                },
                error: function () {
                    toastr.error('Не получилось обновить ячейку сделки', 'Это провал!', {
                        progressBar: true,
                        "timeOut": 3000,
                    });
                }
