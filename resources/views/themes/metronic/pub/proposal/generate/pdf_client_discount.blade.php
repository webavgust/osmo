...
@section('content')
    ...
    <div style="width: 297mm; margin: 0 auto; padding: 10mm 10mm 10mm; box-sizing: border-box;" id="print" contentEditable="true">
        <div class="text-center">
            <h1 class="mb-9" title="{{ $template }}" style="font-size: 2.5rem">{{ __('proposal_pdf.title') }}</h1>
        </div>
        ...
        <p class="my-8">{{ __('proposal_pdf.intro') }}</p>

        <p class="my-8 fw-bold">{{ __('proposal_pdf.end_user') }}: <span keep="company-name">{{ $proposal->company->name ?? '-' }}</span></p>
...
{{--                </div>--}}
        ...
        <div class="pt-4 mt-4">
            <table class="table no-wrap w-100 mb-0" id="table-summary">
                ...
                @foreach($variants as $variant)
                   ...
                   @if($loop->iteration > 1)
                        <tr class="clear"><td colspan="4"/></tr>
                    @endif
                    <tr class="subcaption">
                        <th class="text-wrap" rowspan="2">
                            ...
                            @endif
                            <span class="text-secondary fs-6 fw-normal">({{ __('proposal_pdf.proposal_application') }} {{ $loop->iteration }})</span>
                        </th>
                        <th colspan="4" class="text-center">{{ __('proposal_pdf.proposal_cost_title') }}</th>
                    </tr>
                   <tr class="subcaption">
                        <th width="150" class="text-center py-1">
                            <div class="fs-6">{{ __('proposal_pdf.proposal_amount') }} *</div>
                            <div class="fw-normal fs-8">{{ __('proposal_pdf.proposal_amount_nds') }}</div>
                        </th>
                        <th width="150" class="text-center py-1">
                            <div class="fs-6">{{ __('proposal_pdf.proposal_amount_partner') }} @if($has_partner_discount) ** @endif</div>
                            <div class="fw-normal fs-8">{{ __('proposal_pdf.proposal_amount_nds') }}</div>
                        </th>
                        <th width="120" class="text-center fs-6 py-1" valign="middle">{{ __('proposal_pdf.proposal_tax') }}</th>
                        <th width="150" class="text-center fs-6 py-1">
                            <div class="fs-6">{{ __('proposal_pdf.proposal_total') }} @if($has_partner_discount) ** @endif</div>
                            <div class="fw-normal fs-8">{{ __('proposal_pdf.proposal_total_2') }}</div>
                        </th>
                    </tr>
                    ...
                    @if(!empty($variant->work_cost_total))
                        @php
                            ...
                            $total_cost_client += $variant->work_cost_total_base - $variant->work_discount_customer;
                        @endphp
                        <tr class="fw-bold">
                            <td>{{ __('proposal_pdf.proposal_work') }}</td>
                            <td class="text-end">
                                {!! cost_out($variant->work_cost_total_base - $variant->work_discount_customer, $proposal) !!}
                            </td>
                            <td class="text-end">
                            ...
                            @foreach($groups as $group_name => $works)
                                @php
                                    ...
                                    $cost_base = $works->sum(function($instance) {
                                        return ($instance->count * $instance->cost) * ((100 - $instance->discount_customer) / 100);
                                    });
                                @endphp
                                <tr>
                                    <td class="ps-9 py-3 fs-6">
                ...
                <div keep="{{ $variant->id }}__discount_partner_p">
                    * {{ __('proposal_pdf.proposal_client_discount') }}
                </div>
        ...
        @foreach($variants as $variant)
            ...
            <div class="text-center mt-9">
                <h2 class="mb-6" contentEditable="true">{{ __('proposal_pdf.table_caption') }}</h2>
            </div>
            ...
            <table id="table-summary" class="table no-wrap w-100" keep="main_table">
                ...
                @if($variant->proposal_software->isNotEmpty())
                    <tr class="subcaption">
                        <td></td>
                        <td class="text-center fw-bold fs-6">{{ __('proposal_pdf.tr_platform_title') }}</td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                ...
                @if($variant->proposal_platforms->isNotEmpty())
                    <tr class="subcaption">
                        <td></td>
                        <td class="text-center fw-bold fs-6">{{ __('proposal_pdf.proposal_platform') }}</td>
                        <td></td>
                        <td class="text-center fw-bold fs-6"></td>
                        <td></td>
                        <td class="text-center fw-bold fs-6"></td>
                    </tr>
                    @php
                    ...
                    @foreach($variant->proposal_platforms as $platform)
                        ...
                        <tr @class(["bg-light-warning text-warning" => !$platform->cb_process])>
                            ...
                            <td class="text-center">
                                {!! cost_out(round($platform->cost_discount), $proposal) !!}

                                @if($platform->cost !== $platform->cost_discount)
                                    <div class="text-secondary fs-8">
                                        <span class="text-decoration-line-through">{!! cost_out($platform->cost, $proposal) !!}</span>
                                    </div>
                                @endif
                            </td>
                            <td class="text-center">
                            ...
                            <td class="text-center">
                                @php
                                    $sum = round($platform->cost_discount * $platform->count);
                                    $block_total += $sum;
                                @endphp
                                {!! cost_out($sum, $proposal) !!}
                            </td>
                ...
                @if($variant->proposal_scenarios->isNotEmpty())
                    <tr class="subcaption">
                        <td></td>
                        <td class="text-center fw-bold fs-6 text-wrap">{{ __('proposal_pdf.tr_neuroservices_title') }}</td>
                        <td></td>
                        <td class="text-center fw-bold fs-6">{{ __('proposal_pdf.tr_licenses_title') }}</td>
                        <td></td>
                        <td class="text-center fw-bold fs-6"></td>
                    </tr>
                    @php
                    ...
                    @foreach($variant->proposal_scenarios as $scenario)
                        ...
                        <tr @class(["bg-light-warning text-warning" => !$scenario->cb_process])>
                            ...
                            <td class="text-center">
                                {!! cost_out(round($scenario->cost_discount), $proposal) !!}

                                @if($scenario->cost !== $scenario->cost_discount)
                                    <div class="text-secondary fs-8">
                                        <span class="text-decoration-line-through">{!! cost_out($scenario->cost, $proposal) !!}</span>
                                    </div>
                                @endif
                            </td>
                            <td class="text-center">
                            ...
                            <td class="text-center">
                                @php
                                    $sum = round($scenario->cost_discount * $scenario->count);
                                    $block_total += $sum;
                                @endphp
                                {!! cost_out($sum, $proposal) !!}
                            </td>
                ...
                @if($variant->proposal_works->isNotEmpty())
                    <tr class="subcaption">
                        <td></td>
                        <td class="text-center fw-bold fs-6">{{ __('proposal_pdf.tr_works_title') }}</td>
                        <td></td>
                        <td class="text-center fw-bold fs-6">{{ __('proposal_pdf.tr_hours_title') }}</td>
                        <td></td>
                        <td></td>
                    </tr>
                    @php
                    ...
                    @foreach($variant->proposal_works as $work)
                        ...
                        <tr @class(["bg-light-warning text-warning" => !$work->proposal_work->cb_process])>
                            ...
                            <td @class(["align-center text-center", "text-warning" => !$work->proposal_work->cb_process])>
                                <div>
                                    @if($work->cost_discount)
                                        {!! cost_out(round($work->cost_discount), $proposal) !!}
                                    @else
                                        0 ₽
                                    @endif


                                    @if($work->cost !== $work->cost_discount)
                                        <div class="text-secondary fs-8">
                                            <span class="text-decoration-line-through">{!! cost_out($work->cost, $proposal) !!}</span>
                                        </div>
                                    @endif
                                </div>
                            </td>
                            ...
                            <td class="text-center">
                                @php
                                    $sum = round($work->cost_discount * $work->count);
                                    $block_total += $sum;
                                @endphp
                ...
                <div class="text-center mt-9">
                    <h2 class="mb-6">{{ __('proposal_pdf.hardware_requirements') }}</h2>
                </div>
                ...
                <table id="table-summary" class="table no-wrap w-100" keep="hardware_table">
                    <tr class="caption">
                        <th class="text-center text-dark fw-bold fs-7 p-1" valign="top" width="30">№</th>
                        <th class="text-center text-dark fw-bold fs-7 p-1" valign="top">{{ __('proposal_pdf.tr_name') }}</th>
                        <th class="text-center text-dark fw-bold fs-7 p-1" valign="top">{{ __('proposal_pdf.tr_count_full') }}</th>
                        <th class="text-center text-dark fw-bold fs-7 p-1" valign="top">{{ __('proposal_pdf.tr_model') }}</th>
                    </tr>
                        @forelse($variant->hardware as $hardware)
                            <tr>
                                ...
                                <td class="text-wrap fs-3">{!! $hardware->params !!}</td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="4"/>
            ...
            <table id="table-summary" class="glue table no-wrap w-100 fs-7">
                <tr class="caption">
                ...
                <tr>
                    <td class="text-wrap border-end-0 border-bottom-0">
                         <ol>
                            <li class="ps-2">
                                {{ __('proposal_pdf.payment') }}:
                                <p>
                                    {{ __('proposal_pdf.pre_payment') }}:
                                    <span keep="prepay">{!! cost_out($variant->work_cost_total + $variant->work_nds_cost_total, $proposal) !!}</span>
