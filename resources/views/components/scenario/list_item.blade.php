<div id="{{$scenario->id}}" type="scenario" class="todo-item p-3 border-bottom position-relative group-{{$groupId}} " @if($groupOut != $groupId) style="display: none;" @endif>
                ...
                <div>
                    <div class="content-todo">
                        <h5
                            ...
                            data-todo-header="{{ $scenario->name }}"
                        >
                            {{ $scenario->name }}
                        ...
                        <div>
                            <x-ui.badge.default type="info" class="p-0 d-flex-inline align-items-center py-1">
                                ...
                                <span class="fs-2 me-2">{{ tools()->cost_normalize($scenario->cost['year'] ?? '0') }} ₽</span>
                            </x-ui.badge.default>
                            ...
                            <x-ui.badge.default type="primary" class="p-0 d-flex-inline align-items-center py-1">
                                ...
                                @if($scenario->cost['unlimited'] ?? 0 > 0)
                                    <span class="fs-2 me-2">{{ tools()->cost_normalize($scenario->cost['unlimited'] ?? '0') }} ₽</span>
                                @else
                                    <span class="fs-2 me-2">
                                        {{ \App\Modules\Pub\Constant\Models\Constant::get('scenario_unlimited_multiplier') * 100 }} %
                                    </span>
                                @endif
                            </x-ui.badge.default>
                        </div>
