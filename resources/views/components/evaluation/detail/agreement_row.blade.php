<div id="{{ $user->id }}" num="{{$loop->iteration}}" class="mt-2">
    ...
    @if($evaluation->isNeedAprove() && $user->pivot->agreed === 0 && ($user->id == auth()->user()->id || auth()->user()->isAdmin()))
        <div class="mt-3 mb-3 d-flex">
            <x-ui.button.default btn_type="danger flex-grow-1 me-1" onclick="javascript:agreement_decline({{$user->id}})">
                <x-ui.icon.regular icon="fa-xmark" class="me-1"></x-ui.icon.regular>
                Отказать
            </x-ui.button.default>

            <x-ui.a.default btn_type="success flex-grow-1 ms-1" onclick="javascript:agreement_confirm({{$user->id}})">
