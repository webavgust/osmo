...
<li uid="{{$uid}}">
                ...
                <div class="form-group">
                    <div class="input-group">
                        ...
                        <input type="number" min="0" style="width: 120px" class="form-control inp_cost flex-grow-0"
                               name="address[{{$parent}}][{{$uid}}][expanses]" value="{{$address->expanses}}" placeholder="Расходы">
                   </div>
                </div>
            </div>
        </div>
    ...
    <ul class="points">
        @foreach($address->points as $num => $point)
            <x-order_task.update.point_row parent="{{$uid}}" :point="$point" num="{{$num + 1}}"></x-order_task.update.point_row>
        @endforeach
    </ul>
