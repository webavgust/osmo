...
<div class="d-inline" uuid="{{ $uuid_time }}">
    <input name="time[{{$uuid}}][notificator][]" value="{{ $notificator['type'] }}" type="checkbox" @if(!empty($notificator['default']) && $notificator['default'])  checked @endif class="btn-check" id="cb_{{$uuid_time}}_{{$notificator['type']}}" autocomplete="off">
