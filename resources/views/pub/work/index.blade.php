...
@section('content')
    <div id="filter">
        <button class="btn btn-outline-success d-none" data-bs-toggle="modal" data-bs-target="#filter-modal">
                ...
                <div class="card">
                    <div class="card-body pt-2">
                        <table class="table"
                               ...
                               data-url="{{ route("api.works.list_table", ['_token' => auth()->user()->ajax_token]) }}"
                        ></table>
...
@section('js')
    ...
    <script>
        ...
        function filter_remove() {
            $.ajax({
                url: '{{ route('api.works.filter.remove', ['_token' => auth()->user()->ajax_token]) }}',
                method: 'get',
                dataType: 'json',
        ...
        function filter() {
            $.ajax({
                url: '{{ route('api.works.filter', ['_token' => auth()->user()->ajax_token]) }}',
                method: 'post',
                dataType: 'json',
        ...
        function boxFormatter(value, row) {
            if(!value) {
                ...
                return `<a class="btn waves-effect waves-light btn-outline-info ms-1 py-1 px-2 fs-1" onclick="javascript:box({href: '{{ route('work.box_extended') }}/` + row.id + `'})" btn_type="info" type="button">
        ...
        function row_delete(id) {
            ...
            $.ajax({
                url: "{{ route('api.work.delete') }}/" + id + "?_token=" + csrf_token(),
                type: "DELETE",
                dataType: "json",
                success: function (response) {
                    if (response.result == 'success') {
