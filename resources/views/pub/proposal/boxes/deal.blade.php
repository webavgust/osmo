...
<script>
     ...
     * Закрытие попапа.
     *
     * Если страница умеет перерисовывать ячейку «Сделка» сама (список КП,
     * patch v25) — зовём её и ничего не перезагружаем: список пагинируется
     * на сервере, и перезагрузка сбрасывала бы страницу и сортировку.
     * Остальные страницы обновляются, как раньше.
     */
    function deal_done() {
        box_close();
        if (!deal_dirty) return;

        if (typeof window.deal_changed === 'function') {
            window.deal_changed(@json($proposal->group));
            return;
        }
