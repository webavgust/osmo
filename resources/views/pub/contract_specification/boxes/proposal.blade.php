...
@section('body')
    ...
    <div class="d-flex flex-wrap align-items-center gap-3 mb-4">
        ...
        <div class="ms-auto text-end" style="max-width: 320px">
            <div class="fs-8 text-muted">
                Показаны все КП этой компании, кроме уже прикреплённых к этой спецификации.
            </div>
        </div>
    </div>

    <div class="mb-3">
        <input type="text" class="form-control" id="proposal_search" placeholder="Поиск: номер, название, партнёр, сумма" autocomplete="off">
    </div>

    <table class="table table-bordered align-middle m-0" id="proposal_table">
        @if($proposals->count() > 0)
            ...
            @foreach($proposals as $proposal)
                <tr class="proposal_row" data-search="{{ mb_strtolower($proposal->number . ' ' . $proposal->name . ' ' . ($proposal->partner->name ?? '') . ' ' . round($proposal->cost_total)) }}">
                    <td>
        ...
        @else
            <tr>
                <td class="text-center text-muted py-8">
                    У компании {{ $spec->company->name }} нет КП
    ...
    <script>
        function choose(group, unbind) {            body_block();
            $.ajax({
        ...
        $(document).ready(function () {
            $("#proposal_search").on("keyup", function () {
                var query = $(this).val().toLowerCase().trim();

                $("#proposal_table tr.proposal_row").each(function () {
