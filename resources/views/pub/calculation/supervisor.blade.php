                    ...
                    <div class="card-body">
                        <div class="card-table">
                            ...
                            <x-ui.card.card_table_tr field="Сумма договора">
                                <x-ui.badge.light type="secondary" class="font-16">{{ tools()->cost_normalize($education_task->amount) }} ₽</x-ui.badge.default>
                            </x-ui.card.card_table_tr>

                            <x-ui.card.card_table_tr field="Итого к выплате руководителю">
                                <x-ui.badge.default type="success" class="font-16">{{ tools()->cost_normalize($education_task->supervisor_salary) }} ₽</x-ui.badge.default>
                            </x-ui.card.card_table_tr>

                            @if($education_task->is_tender)
                                <x-ui.card.card_table_tr field="Итого к выплате за тендер">
                                    <a href="{{ route('calculation.tender', $education_task) }}">
                                        <x-ui.badge.default type="secondary" class="font-16">{{ tools()->cost_normalize($education_task->tender_salary) }} ₽</x-ui.badge.default>
                                    </a>
                                </x-ui.card.card_table_tr>
                            @endif
                        </div>
            ...
            <div class="col--12 col-md-7">
                <table class="table customize-table mb-0 v-middle">
                    @foreach($salaries as $salary)
                        @php
                            $calculation_total = 0;
                            $portal_works = $education_task->education_task_works->where('created_at', '<=', $salary->created_at);
                            $calculation_total += $portal_works->sum('total');
                        @endphp
                        ...
                        @foreach($education_task->education_task_courses as $course)
                            @php
                                ...
                                $cost_works = (float)$course->education_task_course_works_our()->where('updated_at', '<', $salary->calculation->created_at)->sum('total');
                            ...
                            <tr class="bg-white">
                                <td>
                                    ...
                                    @if($portal_works->isNotEmpty())
                                        <x-ui.badge.light type="secondary" class="cursor-help" title="Работы на портале" class="me-2">
                                            {{ $portal_works->count() }}
                                            <x-ui.icon.solid icon="fa-helmet-safety"></x-ui.icon.solid>

                                            = {{ tools()->cost_normalize($portal_works->sum('total')) }} ₽
                                        </x-ui.badge.light>
                                    @endif
                                </td>
                                <td class="text-right">
                        ...
                        <tr>
                            <td colspan="100" class="text-right bg-white">
                                <div class="d-inline-flex align-items-center">
                                    ...
                                    @if($portal_works->isNotEmpty())
                                        <span class="px-1">&ndash;</span>
                                        <x-ui.badge.light type="secondary">{{ tools()->cost_normalize($portal_works->sum('total')) }} ₽</x-ui.badge.light>
                                    @endif
                                    <span class="font-20">)</span>
                                    <x-ui.icon.solid icon="fa-xmark" class="font-16 px-2"></x-ui.icon.solid>
                                    <x-ui.badge.light type="primary">{{ $education_task->supervisor_rate }}%</x-ui.badge.light>
                                    <x-ui.icon.solid icon="fa-equals" class="font-16 px-2"></x-ui.icon.solid>
                                    <x-ui.badge.light type="primary" class="font-16">
                                        {{ round(($education_task->amount - ($education_task->amount * (1 / 100 * $education_task->minus_rate)) - $calculation_total) * ($education_task->supervisor_rate / 100), 2) }}
                                    </x-ui.badge.light>
                                    @if($salary->amount > 0)
