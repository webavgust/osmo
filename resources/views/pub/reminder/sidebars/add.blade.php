...
@section('body')
        ...
        <div class="card">
            <div class="card-body p-0">
                        ...
                        <div>
                            ...
                            <select class="form-control select2" multiple name="user[]">
                                @foreach($subUsers as $user)
                                    <option value="{{ $user->id }}" @if($user->id == auth()->id()) selected @endif>{{ $user->fullName }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-check mt-1 mb-3 fs-3">
                            <input name="hide" class="form-check-input" type="checkbox" value="1" id="cb_hide">
                ...
                <div class="mb-1" id="times">
                    <x-reminder.add_time :notificators="$notificators"></x-reminder.add_time>
                </div>
                ...
                <x-ui.button.outline type="button" btn_type="primary" class="mt-3 w-100 disabled" id="remind_submit">
                    Создать напоминание
                </x-ui.button.outline>
            </div>
        </div>
    ...
    <script>
        ...
        function remind_submit(event) {
            ...
            $.ajax({
                url: "{{ route('api.reminder.create', ['_token' => _token()]) }}",
                type: "POST",
                data: $("form#remind").serialize(),
                dataType: "html",
                success: function (html) {
                    $(block_elem).unblock();
                    if($("ul.timeline li:first-of-type").length > 0) {
                        $("ul.timeline li:first-of-type").after(html);
                    } else {
                        location.reload();
                    }
                    sidebar_close();
