...
@section('content')
        <div class="container-fluid">
            <form method="post" action="{{ route('order_task.step1_update', $order_task) }}" id="order_task_create">
                @csrf
            <div class="row">
                <div class="col-12">
                    <div class="card mb-1">
                        ...
                        <div class="card-body p-0 structure">
                            <div class="form-group">
                                <textarea name="contacts" class="form-control" rows="3" placeholder="Контактные данные заказчика">{{$order_task->contacts}}</textarea>
                            </div>
                        </div>
                    </div>
                    <div class="card mb-1 mt-3">
                        <div class="d-flex border-bottom title-part-padding align-items-center justify-content-between">
                            <h4 class="card-title mb-0">Редактирование объектов, адресов и точек</h4>

                            <span class="font-14 badge {{ $order_task->status_decorate['color'] }}">{{ $order_task->status_decorate['name'] }}</span>
                        </div>
                        <div class="card-body p-0 structure">
                                <ul class="objects">
                                    @foreach($order_task->objects as $num => $object)
                                        @component('components.order_task.update.object_row', [
            ...
            <div class="row mt-1">
                ...
                <div class="col-12 col-sm-9 align-items-center justify-content-end d-flex justify-content-between justify-content-sm-end">
                    <x-ui.a.outline btn_type="primary" href="{{ route('order_task.edit_step2', $order_task)  }}"></i> Перейти на 2 шаг <i class="fa-light fa-arrow-right ms-1"></i></x-ui.a.outline>

                    <x-ui.button.default btn_type="success" onclick="javascript:form_confirm();" class="ms-2"><i class="fa-thin fa-circle-plus me-1"></i> Сохранить тех. задание</x-ui.button.default>
                </div>
            </div>
            </form>
        </div>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <p>Вы действительно хотите сохранить первую часть технического задания? После сохранения вы сможете назначить задания созданым точкам.</p>
                    </div>
        ...
        >
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <strong>ВНИМАНИЕ!</strong> Вы удалили объект, адрес или точку: после сохранения все прикреплённые показатели и услуги у данной записи <strong>БУДУТ УДАЛЕНЫ!</strong>
                        <p>Вы действительно хотите сохранить первую часть технического задания? После сохранения вы сможете назначить задания созданым точкам.</p>
                    </div>
