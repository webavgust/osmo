                ...
                <div class="card">
                    <div class="card-body pt-2">
                        <table class="table"
                               ...
                               data-url="{{ route("api.order_task.list_table", ['_token' => auth()->user()->ajax_token]) }}"
                        ></table>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="container">
                            ...
                            <div class="mt-4 row">
                                <label class="col-sm-4 text-end control-label col-form-label"
                                       title="Дата создания">Дата создания</label>
                                <div class="col-sm-8">
                                    <div class="input-group">
                                        <div class="input-group-text">
                                            <div class="form-check">
...
@section('js')
    ...
    <script>
        ...
        function filter_remove() {
            $.ajax({
                url: '{{ route('api.order_task.filter.remove', ['_token' => auth()->user()->ajax_token]) }}',
                method: 'get',
                dataType: 'json',
        ...
        function filter() {
            $.ajax({
                url: '{{ route('api.order_task.filter', ['_token' => auth()->user()->ajax_token]) }}',
                method: 'post',
                dataType: 'json',
        ...
        function contractFormatter(value, row) {
                if(row.sub_contract) {
                    @can('contract_view')
                        return '<a href="{{ route('contract.detail') }}/' + row.sub_contract.contract_id + '">' + row.sub_contract.contract_id + '</a>';
                    @else
                        return row.sub_contract.contract_id;
                    @endcan
                } else {
                    return '';
                }
        ...
        function statusFormatter(value, row) {
            return `
             <span class="mb-1 badge ` + row.status_decorate.color + `">` + row.status_decorate.name + `</span>
            `;
        }
        ...
        function agreementFormatter(value, row) {
            if(row.agreement.name) {
                return `<a href="` + row.agreement.link + `">` + row.agreement.name + `</a>`;
            } else {
                return ``;
            }
        }
        function annexFormatter(value, row) {
            if(row.annex.name) {
                ret = `<div>` + row.annex.name + `</div>`;
                if(row.annex.date)
                    ret += `<div class="fs-1">от ` + row.annex.date+ `</div>`;
        ...
        function btnDetailFormatter(value, row) {
                ...
                return `<a href="{{ route('order_task.detail') }}/` + row.id + `" class="btn waves-effect waves-light btn-outline-primary d-flex align-items-center justify-content-between">
                ...
                <div class="dropdown-action">
                    <div class="dropdown todo-action-dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-right">`;
                            @can('order_task_edit')
                                if(row.can.edit)
                                    ret += `<a class="dropdown-item" href="{{ route('order_task.edit_step1') }}/` + row.id + `"><i class="fas fa-edit text-warning me-2"></i>Редактировать</a>`;
                            @endcan
                            @can('order_task_agree')
                                if(row.can.agree)
                                {
                                    ret += `<a class="dropdown-item agree" href="javascript:sidebar({href:'{{ route('order_task.agreement.form') }}/` + row.id + `'})    "><i class="fas fa-users text-primary me-2"></i>Согласовать</a>`;
                                    ret += `<a class="dropdown-item agree_view d-none" href="javascript:sidebar({href:'{{ route('order_task.agreement.view') }}/` + row.id + `'})    "><i class="fas fa-users text-warning me-2"></i>Согласованты</a>`;
                                } else if (row.can.agree_view) {
                                    ret += `<a class="dropdown-item agree_view" href="javascript:sidebar({href:'{{ route('order_task.agreement.view') }}/` + row.id + `'})    "><i class="fas fa-users text-warning me-2"></i>Согласованты</a>`;
                                }
                            @endif
                            @can('order_task_attach')
                                if(row.can.attach)
                                    ret += `<a class="dropdown-item attach" href="javascript:sidebar({href:'{{ route('order_task.attach.form') }}/` + row.id + `'})    "><i class="fas fa-link text-secondary me-2"></i>Прикрепить</a>`;
                            @endcan
                        ret += `
                        </div>
                    </div>
