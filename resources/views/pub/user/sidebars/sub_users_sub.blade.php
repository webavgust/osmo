...
@section('body')
            ...
            <form method="post" id="agreement">
                    <select class="form-control select2" multiple name="user[]">
                        @foreach($users as $sub)
                            <option value="{{ $sub->id }}" @if($user->sub_users->contains($sub->id)) selected @endif>{{ $sub->fullName }}</option>
                        @endforeach
                    </select>
            </form>
    ...
    <script>
        ...
        function agree_submit() {
            ...
            $.ajax({
                url: "{{ route('users.sub_users_set', [$user, '_token' => _token()]) }}",
                type: "POST",
                dataType: "html",
                data: $("form#agreement").serialize(),
                success: function (result) {
                    $(block_elem).unblock();
                    $(".card.users [block='sub']").html(result);
                    sidebar_close();
                },
                error: function () {
