...
@section('content')
    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-lg-6">
                <x-ui.a.default btn_type="info" href="{{ route('lab-measure.cost_control') }}" class="mb-2">Управлять стоимостью</x-ui.a.default>
                <div class="card card-body" id="catalog-pad">
                    <div id="tree" data-url="{{ route('api.lab-measure.index') }}"></div>
                ...
                <div class="card card-body">
                    <div class="card">
                        <div class="card-body border-bottom">
                            <h4 class="card-title">Редактирование объекта</h4>
                            <h6 class="card-subtitle mb-0">
                                Здесь описание
                            </h6>
                        </div>
                        <form id="form_menu" data-link="{{ route('api.lab-measure.update') }}/{{ old('id') }}" action="" method="POST"
                              class="needs-validation novalidate">
                            @csrf
                            <div class="card-body">
                                <div class="mb-3 row">
                                    <label for="tb-fname"
                                           class="col-sm-4 text-end control-label col-form-label"></label>
                                    <div class="col-sm-4">
                                ...
                                <div class="mb-3 row">
                                    <label for="tb-fname"
                                           class="col-sm-4 text-end control-label col-form-label"><span
                                ...
                                <div class="mb-3 row">
                                    <label for="tb-icon"
                                           class="col-sm-4 text-end control-label col-form-label">Иконка:</label>
                                    <div class="col-sm-4">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class=" modal-header modal-colored-header bg-danger text-white">
                    <h4 class="modal-title" id="danger-header-modalLabel">Добавление пунктов</h4>
                </div>
                <div class="modal-body">
                    <textarea class="form-control" id="massive" rows="10"></textarea>
                </div>
...
@section('js')
    ...
    <script>
                    ...
                    "create_many": {
                        ...
                        "action": function (data) {
                            var inst = $.jstree.reference(data.reference),
                                ...
                                window.create_many_inst = inst;
                                window.create_many_obj = obj;
                        }
                    },
        ...
        function fill_add() {
            ...
            if(text.length > 0)
            {
                $.each(text, function( index, value ) {
                    if(value != '' && value != ' ') {
                        window.create_many_inst.create_node(window.create_many_obj, {text: value}, "last", function (new_node) {

                        });

                        $('#tree').jstree("open_node", create_many_obj);
                    }
                });
            }
            $("#massive").val('');
        ...
        function tree_save() {
            ...
            $.ajax({
                url: "{{ route('api.lab-measure.update') }}",
                global: false,
                type: "POST",
                data: ({
                    data: data,
        ...
        function show_panel(id) {
            $.ajax({
                url: "{{ route('api.lab-measure.view') }}/" + id,
                global: false,
                type: "POST",
                data: ({
                    _token: '{{ auth()->user()->ajax_token }}'
