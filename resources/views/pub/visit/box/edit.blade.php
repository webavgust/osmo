...
@section('body')
                ...
                <div class="mb-3 row">
                    ...
                    <div class="col-sm-8">
                        @can('can_select_sampler')
                            <x-ui.select.multiple id="samplers" name="samplers[]" class="select2" :items="$samplers"
                                                  value-name="full_name" id="id"
                                                  :selected="$visit->users->pluck('id')->toArray()"></x-ui.select.multiple>
                        @else
                ...
                <div class="mb-3 row">
                    ...
                    <div class="col-sm-8">
                        <input type="date" class="form-control" name="date"
                               value="{{ $visit->plan_visit_at->format('Y-m-d') }}" class="w-auto">
                    </div>
                </div>
            </div>
                ...
                <table id="measures" class="table customize-table mb-0 v-middle border-1" border="1">
                    @foreach($address->points as $point)
                        <tbody class="point d-none"
                               samplers="|{{ $point->getSamplers()->pluck('user_id')->implode("|")  }}|">
                        <tr>
                        ...
                        <tr>
                            <td/>
                            <td class="text-center font-10 fw-bold">Выполнено</td>
                        ...
                        @foreach($point->measures as $measure)
                            <tr>
                                ...
                                <td class="text-center ">
                                    <span class="counter cursor-pointer">0 / {{ $measure->count }}</span>
                                </td>
                                <td align="center">
                                    <div class="form-group d-flex align-items-center justify-content-center">
                                        <input name="count[{{ $measure->id }}]" type="number"
                                               class="form-control text-center p-0 font-12" min="0"
                                               limit="{{ $measure->count }}" value="{{ $counts[$measure->id] ?? 0 }}" style="width: 50px">
    ...
    <script>
        function selector_process() {
            ...
            @can('can_select_sampler')
                $.each($("#create_visit .select2").val(), function (key, id) {
        ...
        function count_check() {
            ...
            $.each($("[name='samplers[]']").val(), function(index, value) {
                if(!value)
                    err = true;
            });
            @endcan
        ...
        function save() {
            if (!count_check() || !confirm('Вы действительно хотите сохранить выезд?'))
                return false;

            $("body").block(block_default);
            $.ajax({
                url: '{{ route('api.visit.edit', $visit) }}?_token={{ _token() }}',
