                        ...
                        @foreach($points as $point)
                            ...
                            @foreach($containers[$point->id] as $container)
                                ...
                                @foreach($container->samples as $sample)
                                    ...
                                    <tr class="measure">
                                        @if($loop->iteration == 1)
                                            @if($container->mark == 'self')
                                                <td rowspan="{{ $container->samples->count() }}" class="align-top text-danger">
                                                    Без маркировки
                                                </td>
                                            @else
                                        ...
                                        <td class="p-0">
                                            @foreach($sample->other_assets ?? [] as $asset)
                                                <div
                                                    ...
                                                    <div class="d-flex justify-content-between align-items-center align-items-center p-2 pb-0">

                                                        {{-- Черновик--}}
                                                        @if(!empty($asset->checked_at))
                                                            @if($asset->count !== $asset->count_real)
                                                                <x-ui.badge.light
                                                                    type="danger"
                                                                    class="font-18 fw-bold me-1 cursor-help"
                                                                    style="opacity: .4"
                                                                    title="Было указано"
                                                                >
                                                                    {{ $asset->count }}
                                                            ...
                                                            <x-ui.badge.light
                                                                type="success"
                                                                class="font-18 fw-bold me-3 cursor-help"
                                                        ...
                                                        @else
                                                            <x-ui.badge.light
                                                                :type="$asset->finished_at ? 'warning' : 'secondary'"
                                                        ...
                                                        <div>
                                                            @if(!empty($asset->finished_at))
                                                                <div
                                                                    class="fw-bold font-10 text-nowrap">{{ $asset->finished_at->format('d.m.Y H:i') }}</div>
                                                            @endif

                                                            <div class="fw-bold">
                                                                {{ $asset->creator->last_name }}
                                                            </div>
                                                        </div>
                                                    </div>

                                                    @if($visit->canCheck() && !$asset->checked_at)
                                                        <div class="d-flex justify-content-between align-items-center align-items-center ps-2 py-0 mb-2 mt-1">
                                                                <div id="check_{{ $asset->id }}">
                                                                    <x-visit.asset_check :asset="$asset"/>
                                                                </div>
                                                        </div>
                                                    @endif

                                                    @if(!empty($asset->checked_at) && $asset->count_real !== $asset->count && !empty($asset->comment))
                                                        <x-ui.notification.light type="danger" class="p-1 mx-2 mb-2 fs-1">
                                                            {!! $asset->comment !!}
                                            ...
                                            @if($max > 0 && !$sample->canAssetByUser())
                                                <div class="p-2">
