...
@section('body')
    <form method="POST" id="form">
        <div class="card-body">
            <div class="form-group mb-3 row pb-3">
                ...
                <div class="col-sm-9">
                    <x-ui.select.single name="organization" :items="$organizations" id="id" class="select2" blank-ignore="1" value="{{ $contract->organization->id }}"/>
                </div>
            </div>
            <div class="form-group mb-3 row pb-3">
            ...
            <div class="form-group mb-3 row pb-3">
                ...
                <div class="col-sm-9">
                    <input name="number" type="text" min="0" class="form-control w-50 text-start" id="inp_number" value="{{ $contract->number }}">
                </div>
            </div>
            <div class="form-group mb-3 row pb-3">
                <label for="inp_date" class="col-sm-3 text-end control-label col-form-label">Дата договора</label>
                <div class="col-sm-9">
                    <input name="date" type="date" min="0" class="form-control w-25" id="inp_date" value="{{ $contract->date?->format("Y-m-d") ?? null }}">
                </div>
            </div>
            <div class="form-group mb-3 row pb-3">
                ...
                <div class="col-sm-9">
                    <div class="form-check pt-2">
                        <input class="form-check-input" type="checkbox" value="1" id="inp_signed" name="cb_signed" @checked($contract->cb_signed)>
    ...
    <script>
        ...
        function contract_delete() {
            ...
            $.ajax({
                url: "{{ route('api.contract.delete', [$contract, '_token' => _token() ]) }}",
                type: "DELETE",
                dataType: "json",
        ...
        function save() {
            ...
            $.ajax({
                url: "{{ route('api.contract.update', [$contract, '_token' => _token() ]) }}",
                type: "POST",
                dataType: "json",
        ...
        $(document).ready(function() {
            ...
            }).val("{{ $contract->type }}").trigger('change');
