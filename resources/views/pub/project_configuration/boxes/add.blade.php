...
@section('body')
        ...
        <div class="card-body">
                ...
                <div class="col-sm-9">
                    <select name="data[platform]" class="form-select">
                        @foreach(\App\Modules\Pub\ProjectConfiguration\Models\ProjectConfigurationPlatform::cases() as $platform)
                            <option value="{{ $platform }}" id="{{ $platform->data()['id'] }}">{{ $platform->data()['label'] }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="form-group mb-3 row pb-3">
                ...
                <div class="col-sm-9 d-flex justify-content-start align-items-center">
                    <input name="data[duration]" type="number" value="0" min="0" class="form-control text-start" style="width: 85px" required>
            ...
            <div class="form-group mb-3 row pb-3">
                ...
                <div class="col-sm-9">
                    <input name="data[streams]" type="number" value="1" min="1" class="form-control text-start" style="width: 85px" required>
                </div>
            </div>
            ...
            <div class="form-group mb-3 row pb-3">
                ...
                <div class="col-sm-9">
                    <input name="data[comment]" type="text"  class="form-control text-start" id="inp_number" @if($project->configurations->count() > 0) required @endif >
                </div>
            </div>
    ...
    <script>
        ...
        function save() {
            ...
            $.ajax({
                url: "{{ route('api.project_configuration.add', [$project, '_token' => _token() ]) }}",
                type: "PUT",
                dataType: "json",
