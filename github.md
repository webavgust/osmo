...
- v35: аудит и правка дизайна темы Metronic — слой `osmo-design.css` (чернила ≥ 4.5:1, тёмная тема,
  табличные цифры, фокус), макет PDF из КП, лента напоминаний, панель календаря, телефон
- v36: `ajax.api` без сессии отвечает `{error: 'auth'}` вместо 500 (вход по токену не нужен — решение
  владельца); три обработчика (рабочий график, привязки аналитика) на него больше не показывают успех
- v38: удалены мёртвые страницы «Рабочий график» и «Привязка аналитика к объектам» (маршруты, методы,
  шаблоны, осиротевшие шаблоны `lab`); v37 зарезервирован за синхронизацией Битрикса
- v39: `/api/user/list_table` снова 200 — из `UserRepository::getTable()` убраны фильтр и счётчики удалённых
  связей `groups` / `departments`; таблица «Пользователи» ожила, «null» вместо отчества убран
